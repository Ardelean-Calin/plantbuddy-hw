...
* \section SystemInit  System Initialization
  ...
  - The SystemInit() function is called on BlueNRG-1, BlueNRG-2 main application
as first operation required for properly initialize the device:
    - It remaps the vector table and it configures all the device interrrupt
priorities giving the highest one to the Pendable request for system service and
to the BLE radio BLUE Controller ones.
      - It calls the DeviceConfiguration(TRUE,TRUE) function for performing the
proper application configuration steps.
      - It disables all the  peripherals clock except NVM, SYSCTR, PKA and RNG
         - It's up to user application to enables the other peripherals clock
...
* \section DeviceConfiguration_Cold  Define Application Configurations
  ...
  - During the device initialization phase, some specific parameters must be set
    on BLE device controller registers, in order to define the following
configurations:
    ...
    - BOR configuration.
  - The BlueNRG-1, BlueNRG-2 controller registers values are defined on file
system_bluenrg1.c through the configuration table:
    - COLD_START_CONFIGURATION.
    - This table defines the default configurations as follows:
      - User mode: ATB0_ANA_ENG_REG = USER_MODE_ATB0 (0x00), ATB1_ANA_ENG_REG =
USER_MODE_ATB (0x30)
      - SMPS ON, 10 uH inductor: CLOCK_LOW_ENG_REG = SMPS_ON, RM1_DIG_ENG_REG =
SMPS_10uH_RM1
    ...
    - At device initialization, on reset/power-on, the function SystemInit()
sets the starting parameters defined on the COLD_START_CONFIGURATION table
within the cold_start_config[] array by calling the  DeviceConfiguration(BOOL
coldStart, BOOL waitLS_Ready) function with coldStart = TRUE and waitLS_Ready =
TRUE. As consequence, user application must define the following  preprocessor
options, based on specific application configuration PCB:
      - HS_SPEED_XTAL (high speed cystal: HS_SPEED_XTAL_16MHZ or
HS_SPEED_XTAL_32MHZ)
      - LS_SOURCE     (Low speed crystal source: LS_SOURCE_EXTERNAL_32kHZ,
LS_SOURCE_INTERNAL_RO)
      - SMPS_INDUCTOR (SPMS inductor disabled: SMPS_INDUCTOR_NONE or SPMS
inductor enabled with 10uH: SMPS_INDUCTOR_10uH or SPMS inductor enabled
with 4.7uH: SMPS_INDUCTOR_4_7uH).

      - NOTE: BOR_CONFIG  preprocessor option is set to BOR_ON by default
(brown-out threshold enabled).

    - Regarding the ATB0_ANA_ENG_REG, ATB1_ANA_ENG_REG registers settings, some
test modes are also available in order to address some test scenarios.

      WARNING: when using one of the test modes DIO14 cannot be used and must be
kept configured as an input pin.
      ...
      User should sets such registers as follows:
        ...
        - Please notice that the default user mode register setting must be
restored for typical user mode application scenarios:
...
* \section General_Configuration  General Device Configuration
 ...
 - Beyond the application configuration performed only  at device reset/power-on
by calling the DeviceConfiguration() function with coldStart = TRUE, this
function performs the following general configuration operations:
...
* \section DeviceConfiguration_ALL  How to set Device Configuration Parameters?
  ...
  - The selected device configuration parameters  are set within the BLE device
controller, Radio configuration register,  through the following instructions
executed on DeviceConfiguration() function:

        BLUE_CTRL->RADIO_CONFIG = 0x10000U |
(uint16_t)((uint32_t)cold_start_config & 0x0000FFFFU); while
((BLUE_CTRL->RADIO_CONFIG & 0x10000) != 0);

  - Once the device configuration parameters are set, the DeviceConfiguration()
function must wait until the High Speed (HS) crystal is ready. Since the slow
clock period measurement is done automatically each time the device enters in
active2 state and the HS cystal is ready, the related Clock Gen interrupt
signals that a slow clock period measurement has been done:

        while(CKGEN_BLE->CLK32K_IT == 0); //Interrupt event for 32 kHz clock
measurement
 ...
 *  necessary.
 *  If this flag is non-zero, the WFI instruction puts the BlueNRG-1 in deep
 * sleep. So, at wakeup time a context restore is necessary. Note: The smallest
 * unit of storage is a single byte.
 *
...
extern unsigned long _sidata;   /* start address for the initialization values of
                                   the .data section. Defined in linker script */
extern unsigned long _sdata;    /* start address for the .data section. Defined in linker script */
extern unsigned long _edata;    /* end address for the .data section. Defined in linker script */
extern unsigned long _sbss;     /* start address for the .bss section. Defined in linker script */
extern unsigned long _ebss;     /* end address for the .bss section. Defined in linker script */
extern unsigned long _sbssblue; /* start address for the section reserved for the Blue
                                   controller. Defined in linker script */
extern unsigned long _ebssblue; /* end address for the section reserved for the
                                   Blue controller. Defined in linker script */
extern unsigned long _estack;   /* init value for the stack pointer. Defined in linker script */
extern unsigned long _sidata2;  /* start address for the initialization values of
                                   the special ram_preamble */
extern unsigned long _sdata2;   /* start address the special ram_preamble defined
                                   in linker script */
...
void RESET_HANDLER(void)
{
  if (__low_level_init() == 1)
  {
    ...
    pulSrc = &_sidata2;
    for (pulDest = &_sdata2; pulDest < &_edata2;) { *(pulDest++) = *(pulSrc++); }

    // Zero fill the bss segment.
    for (pulDest = &_sbssblue; pulDest < &_ebssblue;) { *(pulDest++) = 0; }
...
REQUIRED(__blue_RAM_struct __blue_RAM) = {
    0,
};
...
#define COLD_START_CONFIGURATION                                                                \
  {                                                                                             \
    NUMBER_CONFIG_BYTE, ATB0_ANA_ENG_REG, USER_MODE_ATB0, NUMBER_CONFIG_BYTE, ATB1_ANA_ENG_REG, \
        USER_MODE_ATB1, NUMBER_CONFIG_BYTE, RM1_DIG_ENG_REG, SMPS_10uH_RM1, NUMBER_CONFIG_BYTE, \
        CLOCK_LOW_ENG_REG, SMPS_ON, NUMBER_CONFIG_BYTE, CLOCK_HIGH_ENG_REG, HIGH_FREQ_16M,      \
        NUMBER_CONFIG_BYTE, PMU_ANA_ENG_REG, SMPS_10uH_PMU, NUMBER_CONFIG_BYTE,                 \
        CLOCK_ANA_USER_REG, LOW_FREQ_XO, NUMBER_CONFIG_BYTE, PMU_ANA_USER_REG,                  \
        PMU_ANA_USER_RESET_VALUE, END_CONFIG                                                    \
  }
...
void Set_RF_FrontEnd(void)
{
  ...
  while (hot_table_radio_config[i] != 0) i += (hot_table_radio_config[i] + 1);
  ...
  if ((i + 3) < HOT_TABLE_RADIO_SIZE)
  {
    ...
    BLUE_CTRL->RADIO_CONFIG =
        0x10000U | (uint16_t)((uint32_t)(&hot_table_radio_config[i]) & 0x0000FFFFU);
    while ((BLUE_CTRL->RADIO_CONFIG & 0x10000) != 0)
      ;
  }
}

/* Function to store the BOR configuration and to populate the hot radio
 * configuration table */
void BOR_ConfigSave(uint8_t pmu_ana_user_conf)
{
  ...
  if ((i + 3) < HOT_TABLE_RADIO_SIZE)
  {
    /* BOR configuration storage inside hot table used only for smart power
     * managment */
    hot_table_radio_config[i]     = NUMBER_CONFIG_BYTE;
...
/* Function to access the BLUE SPI registers to disable and restore the BOR
 * configuration */
...
#warning "No definition for Low Speed Crystal Source"
    ...
    if ((Trimm_config >> 16) == CHECK_BYTES)
    {
      ...
      cold_start_config[23] |= (Trimm_config & 0x7)
                               << 4; // Store the RCO32K trimming value in bit 4-5-6
    }
    ...
    if ((Trimm_config >> 16) == CHECK_BYTES)
    {
      ...
      cold_start_config[5] |= (Trimm_config & 0x3)
                              << 4; // Store the LDO1V2 trimming value in bit 4 and 5
    }
    ...
    if ((Trimm_config >> 16) == CHECK_BYTES)
    {
      ...
      cold_start_config[23] |=
          (Trimm_config & 0x3); // Store the BLD_LVL trimming value in bit 0 and 1
    }

    /* BOR configuration. Note: this setup shall be executed after the SMPS
     * configuration*/
