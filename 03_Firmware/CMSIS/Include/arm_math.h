...
/**
   ...
   * Initialize macro __FPU_PRESENT = 1 when building on FPU supported Targets.
   Enable this macro for M4bf and M4lf libraries
   *
   ...
   * - arm_cortexM4lf_math.lib (Little endian and Floating Point Unit on
   Cortex-M4)
   ...
   * Simply include this file and link the appropriate library in the
   ...
   single
   * public header file <code> arm_math.h</code> for Cortex-M4/M3/M0 with little
   ...
   *
   * The library installer contains project files to re build libraries on MDK
   Tool chain in the <code>CMSIS\\DSP_Lib\\Source\\ARM</code> folder.
   ...
   *
   *
   * The project can be built by opening the appropriate project in MDK-ARM 4.23
   chain and defining the optional pre processor MACROs detailed above.
   *
...
#define ALIGN4 __align(4)
  ...
  typedef enum
  {
    ...
    ARM_MATH_SINGULAR       = -5, /**< Generated by matrix inversion if the input
...
/* No need to redefine __CLZ */
  ...
  __STATIC_INLINE q31_t __SSAT(q31_t x, uint32_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __QADD8(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __QSUB8(q31_t x, q31_t y)
  {
   ...
   * @brief C custom defined QADD16 for M3 and M0 processors
   */
  __STATIC_INLINE q31_t __QADD16(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __SHADD16(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __QSUB16(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __SHSUB16(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __QASX(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __SHASX(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __QSAX(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __SHSAX(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __SMUSDX(q31_t x, q31_t y)
  {

    return ((q31_t)(((short)x * (short)(y >> 16)) - ((short)(x >> 16) * (short)y)));
  }

  /*
   * @brief C custom defined SMUADX for M3 and M0 processors
   */
  __STATIC_INLINE q31_t __SMUADX(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __QADD(q31_t x, q31_t y) { return clip_q63_to_q31((q63_t)x + y); }

  /*
   * @brief C custom defined QSUB for M3 and M0 processors
   */
  __STATIC_INLINE q31_t __QSUB(q31_t x, q31_t y) { return clip_q63_to_q31((q63_t)x - y); }

  /*
   * @brief C custom defined SMLAD for M3 and M0 processors
   */
  __STATIC_INLINE q31_t __SMLAD(q31_t x, q31_t y, q31_t sum)
  {
   ...
   */
  __STATIC_INLINE q31_t __SMLADX(q31_t x, q31_t y, q31_t sum)
  {
   ...
   */
  __STATIC_INLINE q31_t __SMLSDX(q31_t x, q31_t y, q31_t sum)
  {
   ...
   */
  __STATIC_INLINE q63_t __SMLALD(q31_t x, q31_t y, q63_t sum)
  {
   ...
   */
  __STATIC_INLINE q63_t __SMLALDX(q31_t x, q31_t y, q63_t sum)
  {
   ...
   */
  __STATIC_INLINE q31_t __SMUAD(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __SMUSD(q31_t x, q31_t y)
  {
   ...
   */
  __STATIC_INLINE q31_t __SXTB16(q31_t x)
  {
  ...
  typedef struct
  {
    ...
    q7_t*    pState;  /**< points to the state variable array. The array is of
  ...
  {
    uint16_t numTaps; /**< number of filter coefficients in the filter. */
    q15_t*   pState;  /**< points to the state variable array. The array is of
                         length numTaps+blockSize-1. */
  ...
  {
    uint16_t numTaps; /**< number of filter coefficients in the filter. */
    q31_t*   pState;  /**< points to the state variable array. The array is of
                         length numTaps+blockSize-1. */
   ...
   * @param[in] 	*pCoeffs points to the filter coefficients.
   * @param[in] 	*pState points to the state buffer.
   * @param[in] 	blockSize number of samples that are processed at a
   * time.
   * @return 		none.
   */
  ...
  typedef struct
  {
    uint32_t numStages; /**< number of 2nd order stages in the filter.  Overall
                           order is 2*numStages. */
    float32_t* pState;  /**< Points to the array of state coefficients.  The
                           array is of length 4*numStages. */
    float32_t* pCoeffs; /**< Points to the array of coefficients.  The array is
                           of length 5*numStages. */
   ...
   * @param[in]     *pCoeffs     points to the filter coefficients.
   * @param[in]     *pState      points to the state buffer.
   * @param[in]     postShift    Shift to be applied to the output. Varies
   ...
   * <code>ARM_MATH_SIZE_MISMATCH</code> or <code>ARM_MATH_SUCCESS</code> based
   * on the outcome of size checking.
   */
   ...
   * @param[in]       *pSrcB  points to the second input matrix structure
   * @param[out]      *pDst   points to output matrix structure
   * @param[in]		  *pState points to the array for storing intermediate
   * results
   * @return     The function returns either
   * <code>ARM_MATH_SIZE_MISMATCH</code> or <code>ARM_MATH_SUCCESS</code> based
   * on the outcome of size checking.
  ...
  void arm_mat_init_q31(arm_matrix_instance_q31* S, uint16_t nRows, uint16_t nColumns,
                        q31_t* pData);
  ...
  void arm_mat_init_q15(arm_matrix_instance_q15* S, uint16_t nRows, uint16_t nColumns,
                        q15_t* pData);
   ...
   * @param[in,out] *S is an instance of the floating-point PID Control
   * structure
   * @return none
   */
   ...
   * @param[in,out] *S points to an instance of the Q15 PID structure.
   * @param[in] resetStateFlag  flag to reset the state. 0 = no change in state
   * 1 = reset the state.
   * @return none.
   */
  ...
  /**
   * @brief Instance structure for the floating-point Linear Interpolate
   * function.
   */
  typedef struct
  {
  ...
  typedef struct
  {
    ...
    uint16_t  twidCoefModifier; /**< twiddle coefficient modifier that supports
                                   ...
                                   factor table. */
    uint16_t bitRevFactor;      /**< bit reversal modifier that supports different
   ...
   * @param[in]      *S    points to an instance of the Q15 CFFT/CIFFT
   * structure.
   ...
   * initialization is successful or ARM_MATH_ARGUMENT_ERROR if
   ...
   * @param[in]      *S    points to an instance of the Q31 CFFT/CIFFT
   * structure.
   * @param[in, out] *pSrc points to the complex data buffer. Processing occurs
   * in-place.
   * @return none.
   ...
   * @param[in]      *S    points to an instance of the Radix-2 Q31 CFFT/CIFFT
   * structure.
   * @param[in, out] *pSrc points to the complex data buffer. Processing occurs
   * in-place.
   * @return none.
   ...
   * @param[in,out] *S             points to an instance of the Radix-2 Q31
  ...
  void arm_radix4_butterfly_f32(float32_t* pSrc, uint16_t fftLen, float32_t* pCoef,
  ...
  void arm_radix4_butterfly_inverse_f32(float32_t* pSrc, uint16_t fftLen, float32_t* pCoef,
  ...
  void arm_bitreversal_f32(float32_t* pSrc, uint16_t fftSize, uint16_t bitRevFactor,
                           uint16_t* pBitRevTab);
   ...
   * @param[in, out] *pSrc            points to the in-place buffer of f32 data
   * type.
   * @param[in]      fftLen           length of the FFT.
   * @param[in]      *pCoef        	  points to Twiddle coefficient buffer.
   * @param[in]      twidCoefModifier twiddle coefficient modifier that supports
  ...
  void arm_radix2_butterfly_f32(float32_t* pSrc, uint32_t fftLen, float32_t* pCoef,
                                uint16_t twidCoefModifier);
   ...
   * @param[in, out] *pSrc            points to the in-place buffer of Q15 data
   * type.
   * @param[in]      fftLen           length of the FFT.
   * @param[in]      *pCoef           points to Twiddle coefficient buffer.
   * @param[in]      twidCoefModifier twiddle coefficient modifier that supports
  ...
  void arm_radix2_butterfly_inverse_q15(q15_t* pSrc, uint32_t fftLen, q15_t* pCoef,
                                        uint16_t twidCoefModifier);
  ...
  void arm_radix2_butterfly_inverse_q31(q31_t* pSrc, uint32_t fftLen, q31_t* pCoef,
                                        uint16_t twidCoefModifier);
  ...
  void arm_radix2_butterfly_inverse_f32(float32_t* pSrc, uint32_t fftLen, float32_t* pCoef,
                                        uint16_t twidCoefModifier, float32_t onebyfftLen);
  ...
  void arm_radix4_butterfly_inverse_q31(q31_t* pSrc, uint32_t fftLen, q31_t* pCoef,
                                        uint32_t twidCoefModifier);
  ...
  void arm_radix4_butterfly_inverse_q15(q15_t* pSrc16, uint32_t fftLen, q15_t* pCoef16,
                                        uint32_t twidCoefModifier);
  ...
  typedef struct
  {
    ...
    uint8_t bitReverseFlagR;    /**< flag that enables (bitReverseFlagR=1) or disables
   ...
   * structure.
   * @param[in]      *S_CFFT        points to an instance of the Q15 CFFT/CIFFT
   * structure.
   ...
   * structure.
   * @param[in, out] *S_CFFT        points to an instance of the Q31 CFFT/CIFFT
   * structure.
   * @param[in]      fftLenReal     length of the FFT.
   * @param[in]      ifftFlagR      flag that selects forward (ifftFlagR=0) or
   * inverse (ifftFlagR=1) transform.
   ...
   * @param[in,out] *S         points to an instance of floating-point
   * DCT4/IDCT4 structure.
   * @param[in]     *S_RFFT    points to an instance of floating-point
   * RFFT/RIFFT structure.
   * @param[in]     *S_CFFT    points to an instance of floating-point
   * CFFT/CIFFT structure.
   * @param[in]     N          length of the DCT4.
   * @param[in]     Nby2       half of the length of the DCT4.
   * @param[in]     normalize  normalizing factor.
   * @return		arm_status function returns ARM_MATH_SUCCESS if initialization
   * is successful or ARM_MATH_ARGUMENT_ERROR if <code>fftLenReal</code> is not
   ...
   * @param[in,out] *S         points to an instance of Q31 DCT4/IDCT4
   * structure.
   ...
   * @param[in]     normalize  normalizing factor.
   * @return		arm_status function returns ARM_MATH_SUCCESS if initialization
   * is successful or ARM_MATH_ARGUMENT_ERROR if <code>N</code> is not a
   ...
   * @param[in,out] *S         points to an instance of Q15 DCT4/IDCT4
   * structure.
   * @param[in]     *S_RFFT    points to an instance of Q15 RFFT/RIFFT
   * structure.
   * @param[in]     *S_CFFT    points to an instance of Q15 CFFT/CIFFT
   * structure.
   * @param[in]     N          length of the DCT4.
   * @param[in]     Nby2       half of the length of the DCT4.
   * @param[in]     normalize  normalizing factor.
  ...
  void arm_conv_fast_q15(q15_t* pSrcA, uint32_t srcALen, q15_t* pSrcB, uint32_t srcBLen,
                         q15_t* pDst);
  ...
  void arm_conv_fast_q31(q31_t* pSrcA, uint32_t srcALen, q31_t* pSrcB, uint32_t srcBLen,
                         q31_t* pDst);
   ...
   * correctly or ARM_MATH_ARGUMENT_ERROR if the requested subset is not in the
   * range [0 srcALen+srcBLen-2].
   */
  ...
  arm_status arm_conv_partial_opt_q15(q15_t* pSrcA, uint32_t srcALen, q15_t* pSrcB,
  ...
  /**
   * @brief Partial convolution of Q15 sequences (fast version) for Cortex-M3
   * and Cortex-M4
   * @param[in]       *pSrcA points to the first input sequence.
   * @param[in]       srcALen length of the first input sequence.
   * @param[in]       *pSrcB points to the second input sequence.
  ...
  arm_status arm_conv_partial_fast_q15(q15_t* pSrcA, uint32_t srcALen, q15_t* pSrcB,
                                       uint32_t srcBLen, q15_t* pDst, uint32_t firstIndex,
                                       uint32_t numPoints);
  ...
  /**
   * @brief Partial convolution of Q31 sequences (fast version) for Cortex-M3
   * and Cortex-M4
   * @param[in]       *pSrcA points to the first input sequence.
   * @param[in]       srcALen length of the first input sequence.
   * @param[in]       *pSrcB points to the second input sequence.
  ...
  arm_status arm_conv_partial_fast_q31(q31_t* pSrcA, uint32_t srcALen, q31_t* pSrcB,
   ...
   * @param[in] *S points to an instance of the floating-point FIR decimator
   * structure.
   * @param[in] *pSrc points to the block of input data.
   * @param[out] *pDst points to the block of output data
  ...
  void arm_fir_decimate_f32(const arm_fir_decimate_instance_f32* S, float32_t* pSrc,
                            float32_t* pDst, uint32_t blockSize);
  ...
  arm_status arm_fir_decimate_init_f32(arm_fir_decimate_instance_f32* S, uint16_t numTaps,
                                       uint8_t M, float32_t* pCoeffs, float32_t* pState,
                                       uint32_t blockSize);
  ...
  arm_status arm_fir_decimate_init_q15(arm_fir_decimate_instance_q15* S, uint16_t numTaps,
                                       uint8_t M, q15_t* pCoeffs, q15_t* pState,
                                       uint32_t blockSize);
  ...
  arm_status arm_fir_decimate_init_q31(arm_fir_decimate_instance_q31* S, uint16_t numTaps,
                                       uint8_t M, q31_t* pCoeffs, q31_t* pState,
                                       uint32_t blockSize);
  ...
  typedef struct
  {
    ...
    q15_t*   pCoeffs;     /**< points to the coefficient array. The array is of length
                             L*phaseLength. */
    q15_t* pState;        /**< points to the state variable array. The array is of
                             length blockSize+phaseLength-1. */
   ...
   * @param[in]     blockSize number of input samples to process per call.
   * @return        The function returns ARM_MATH_SUCCESS if initialization is
   * successful or ARM_MATH_LENGTH_ERROR if the filter length
  ...
  typedef struct
  {
    uint8_t numStages;  /**< number of 2nd order stages in the filter.  Overall
                           order is 2*numStages. */
    float32_t* pState;  /**< points to the array of state coefficients.  The
                           array is of length 2*numStages. */
    float32_t* pCoeffs; /**< points to the array of coefficients.  The array is
                           of length 5*numStages. */
  ...
  /**
   * @brief  Initialization function for the floating-point transposed direct
  ...
  typedef struct
  {
    ...
    q15_t* pkCoeffs;    /**< points to the reflection coefficient array. The array
                           is of length numStages. */
    q15_t* pvCoeffs;    /**< points to the ladder coefficient array. The array is
  ...
  typedef struct
  {
    ...
    q31_t*   pState;    /**< points to the state variable array. The array is of
                           length numStages+blockSize. */
    q31_t* pkCoeffs;    /**< points to the reflection coefficient array. The array
                           is of length numStages. */
    q31_t* pvCoeffs;    /**< points to the ladder coefficient array. The array is
                           of length numStages+1. */
  ...
  typedef struct
  {
    ...
    float32_t* pvCoeffs;  /**< points to the ladder coefficient array. The array
                             is of length numStages+1. */
   ...
   * @param[in] *pkCoeffs points to the reflection coefficient buffer.  The
   * array is of length numStages.
  ...
  void arm_iir_lattice_init_q31(arm_iir_lattice_instance_q31* S, uint16_t numStages,
                                q31_t* pkCoeffs, q31_t* pvCoeffs, q31_t* pState,
                                uint32_t blockSize);
   ...
   * @param[in] *S points to an instance of the fixed-point Q15 IIR lattice
   * structure.
   * @param[in] numStages  number of stages in the filter.
  ...
  void arm_lms_q15(const arm_lms_instance_q15* S, q15_t* pSrc, q15_t* pRef, q15_t* pOut,
  ...
  void arm_lms_q31(const arm_lms_instance_q31* S, q31_t* pSrc, q31_t* pRef, q31_t* pOut,
  ...
  void arm_correlate_q15(q15_t* pSrcA, uint32_t srcALen, q15_t* pSrcB, uint32_t srcBLen,
                         q15_t* pDst);
  ...
  void arm_correlate_q31(q31_t* pSrcA, uint32_t srcALen, q31_t* pSrcB, uint32_t srcBLen,
                         q31_t* pDst);
  ...
  {
    uint16_t numTaps;    /**< number of coefficients in the filter. */
    uint16_t stateIndex; /**< state buffer index.  Points to the oldest sample
                            in the state buffer. */
    ...
    int32_t* pTapDelay;  /**< points to the array of delay values.  The array is
                            of length numTaps. */
   ...
   * @param[in,out] *S         points to an instance of the floating-point
   * sparse FIR structure.
   * @param[in]     numTaps    number of nonzero coefficients in the filter.
   * @param[in]     *pCoeffs   points to the array of filter coefficients.
   * @param[in]     *pState    points to the state buffer.
   * @param[in]     *pTapDelay points to the array of offset times.
   * @param[in]     maxDelay   maximum offset time supported.
   * @param[in]     blockSize  number of samples that will be processed per
   * block.
   * @return none
   */
  ...
  void arm_fir_sparse_q31(arm_fir_sparse_instance_q31* S, q31_t* pSrc, q31_t* pDst,
  ...
  void arm_fir_sparse_q15(arm_fir_sparse_instance_q15* S, q15_t* pSrc, q15_t* pDst,
   ...
   * processed value. <code>S</code> points to an instance of the PID control
   * data structure.  <code>in</code> is the input sample value. The functions
   ...
   *
   * \par
   * where \c Kp is proportional constant, \c Ki is Integral constant and \c Kd
   ...
   * current error, the integral value determines the reaction based on the sum
   * of recent errors, and the derivative value determines the reaction based on
   ...
   * Controller functions. In particular, the overflow and saturation behavior
   ...
   * structure.
   * @return The function returns ARM_MATH_SIZE_MISMATCH, if the dimensions do
   * not match. If the input matrix is singular (does not have an inverse), then
   * the algorithm terminates and returns error status ARM_MATH_SINGULAR.
   */
   ...
   * three-phase currents <code>Ia, Ib and Ic</code> to calculate currents in
   * the two-phase orthogonal stator axis <code>Ialpha</code> and
   * <code>Ibeta</code>. When <code>Ialpha</code> is superposed with
   * <code>Ia</code> as shown in the figure below \image html clarke.gif Stator
   * current space vector and its components in (a,b). and <code>Ia + Ib + Ic =
   * 0</code>, in this condition <code>Ialpha</code> and <code>Ibeta</code> can
   * be calculated using only <code>Ia</code> and <code>Ib</code>.
   *
   ...
   * instantaneous stator phases and <code>pIalpha</code> and
   * <code>pIbeta</code> are the two coordinates of time invariant vector. \par
   * Fixed-Point Behavior Care must be taken when using the Q31 version of the
   * Clarke transform. In particular, the overflow and saturation behavior of
   * the accumulator used must be considered. Refer to the function specific
   * documentation below for usage guidelines.
   */
   ...
   * @param[out]      *pIalpha points to output two-phase orthogonal vector axis
   * alpha
   * @param[out]      *pIbeta  points to output two-phase orthogonal vector axis
   * beta
   * @return none.
   */

  __STATIC_INLINE void arm_clarke_f32(float32_t Ia, float32_t Ib, float32_t* pIalpha,
                                      float32_t* pIbeta)
  {
   ...
   * @param[out]      *pIa    points to output three-phase coordinate
   * <code>a</code>
   * @param[out]      *pIb    points to output three-phase coordinate
   * <code>b</code>
   * @return none.
   */

  __STATIC_INLINE void arm_inv_clarke_f32(float32_t Ialpha, float32_t Ibeta, float32_t* pIa,
                                          float32_t* pIb)
  {
   ...
   * transformation of the <code>Ialpha</code> and the <code>Ibeta</code>
   * currents from the stationary to the moving reference frame and control the
   * spatial relationship between the stator vector current and rotor flux
   * vector. If we consider the d axis aligned with the rotor flux, the diagram
   * below shows the current vector and the relationship from the two reference
   * frames: \image html park.gif "Stator current space vector and its component
   * in (a,b) and in the d,q rotating reference frame"
   *
   * The function operates on a single sample of data and each call to the
   * function returns the processed output. The library provides separate
   * functions for Q31 and floating-point data types. \par Algorithm \image html
  ...
  __STATIC_INLINE void arm_park_f32(float32_t Ialpha, float32_t Ibeta, float32_t* pId,
                                    float32_t* pIq, float32_t sinVal, float32_t cosVal)
  {
    ...
    *pId = Ialpha * cosVal + Ibeta * sinVal;
    ...
    /* Calculate pIq using the equation, pIq = - Ialpha * sinVal + Ibeta *
     * cosVal */
  ...
  __STATIC_INLINE void arm_park_q31(q31_t Ialpha, q31_t Ibeta, q31_t* pId, q31_t* pIq, q31_t sinVal,
                                    q31_t cosVal)
  {
    q31_t product1, product2; /* Temporary variables used to store intermediate results */
   ...
   * function returns the processed output. The library provides separate
   * functions for Q31 and floating-point data types. \par Algorithm \image html
   * parkInvFormula.gif where <code>pIalpha</code> and <code>pIbeta</code> are
   * the stator vector components, <code>Id</code> and <code>Iq</code> are rotor
   * vector components and <code>cosVal</code> and <code>sinVal</code> are the
   * cosine and sine values of theta (rotor flux position).
   * \par Fixed-Point Behavior
   * Care must be taken when using the Q31 version of the Park transform.
   ...
   * @param[out]      *pIalpha  points to output two-phase orthogonal vector
   * axis alpha
   * @param[out]      *pIbeta   points to output two-phase orthogonal vector
   * axis beta
   * @param[in]       sinVal    sine value of rotation angle theta
   * @param[in]       cosVal    cosine value of rotation angle theta
   * @return none.
  ...
  __STATIC_INLINE void arm_inv_park_f32(float32_t Id, float32_t Iq, float32_t* pIalpha,
  ...
  __STATIC_INLINE void arm_inv_park_q31(q31_t Id, q31_t Iq, q31_t* pIalpha, q31_t* pIbeta,
   ...
   * Linear interpolation works by effectively drawing a straight line between
   * two neighboring samples and returning the appropriate point along that line
   *
   * \par
   ...
   * input(x) using linear interpolation of the input values x0, x1( nearest
   * input values) and the output values y0 and y1(nearest output values)
   *
   * \par Algorithm:
   * <pre>
   ...
   * for Q7, Q15, Q31, and floating-point data types.  The functions operate on
   * a single sample of data and each call to the function returns a single
   ...
   * value of the table if x is below input range and returns last value of
   * table if x is above range.
   */
  ...
  __STATIC_INLINE float32_t arm_linear_interp_f32(arm_linear_interp_instance_f32* S, float32_t x)
  {
    ...
    {
      /* Iniatilize output for above specified range as last output value of
       * table */
  ...
  __STATIC_INLINE void arm_circularWrite_f32(int32_t* circBuffer, int32_t L, uint16_t* writeOffset,
   ...
   */
  __STATIC_INLINE void arm_circularRead_f32(int32_t* circBuffer, int32_t L, int32_t* readOffset,
  ...
  __STATIC_INLINE void arm_circularWrite_q15(q15_t* circBuffer, int32_t L, uint16_t* writeOffset,
   ...
   */
  __STATIC_INLINE void arm_circularRead_q15(q15_t* circBuffer, int32_t L, int32_t* readOffset,
  ...
  __STATIC_INLINE void arm_circularWrite_q7(q7_t* circBuffer, int32_t L, uint16_t* writeOffset,
   ...
   */
  __STATIC_INLINE void arm_circularRead_q7(q7_t* circBuffer, int32_t L, int32_t* readOffset,
   ...
   * two dimensional grid. The underlying function <code>f(x, y)</code> is
   * sampled on a regular grid and the interpolation process determines values
   * between the grid points. Bilinear interpolation is equivalent to two step
   * linear interpolation, first in the x-dimension and then in the y-dimension.
   * Bilinear interpolation is often used in image processing to rescale images.
   * The CMSIS DSP library provides bilinear interpolation functions for Q7,
   * Q15, Q31, and floating-point data types.
   *
   * <b>Algorithm</b>
   * \par
   * The instance structure used by the bilinear interpolation functions
   * describes a two dimensional data table. For floating-point, the instance
   * structure is defined as: <pre> typedef struct
   *   {
   *     uint16_t numRows;
   *     uint16_t numCols;
   *     float32_t *pData;
   ...
   * organized in row order and the supplied data values fall on integer
   * indexes. That is, table element (x,y) is located at <code>pTable[x +
   * y*numCols]</code> where x and y are integers.
   *
   * \par
  ...
  __STATIC_INLINE q31_t arm_bilinear_interp_q31(arm_bilinear_interp_instance_q31* S, q31_t X,
                                                q31_t Y)
  {
  ...
  __STATIC_INLINE q15_t arm_bilinear_interp_q15(arm_bilinear_interp_instance_q15* S, q31_t X,
                                                q31_t Y)
  {
    q63_t    acc = 0;             /* output */
    q31_t    out;                 /* Temporary output */
