 ...
 */
#define MBLOCKS_CALC(pw, mtu, n_link) \
  ((pw) + MAX(MEM_BLOCK_X_MTU(mtu, n_link), (MBLOCKS_SECURE_CONNECTIONS)))
 ...
 */
#define DEFAULT_MBLOCKS_COUNT \
  MBLOCKS_CALC(DEFAULT_PREP_WRITE_LIST_SIZE, DEFAULT_MAX_ATT_MTU, NUM_LINKS)
...
/**
 *
 * This macro returns the amount of memory, in bytes, needed for the storage of GATT database
 * elements and other data structures whose size depends on the number of supported connections.
 *
 * @param NUM_LINKS: Maximum number of simultaneous connections that the device will support. Valid
 * values are from 1 to 8.
 * @param NUM_GATT_ATTRIBUTES: Maximum number of Attributes (i.e. the number of characteristic + the
 * number of characteristic values + the number of descriptors, excluding the services) that can be
 * stored in the GATT database. Note that certain characteristics and relative descriptors are added
 * automatically during device initialization so this parameters should be 9 plus the number of user
 * Attributes
 * @param NUM_GATT_SERVICES: Maximum number of Services that can be stored in the GATT database.
 ...
 * @param D_LEN_EXT_EN: Enable or disable the Extended Packet length feature. Valid values are 0
 * or 1.
 */
#define TOTAL_BUFFER_SIZE(NUM_LINKS,                                                    \
                          NUM_GATT_ATTRIBUTES,                                          \
                          NUM_GATT_SERVICES,                                            \
                          ATT_VALUE_ARRAY_SIZE,                                         \
                          MBLOCKS_COUNT,                                                \
                          D_LEN_EXT_EN)                                                 \
  (FIXED_BUFFER_SIZE_BYTES + ((((ATT_VALUE_ARRAY_SIZE)-1) | 3) + 1) +                   \
   (VARIABLE_BUFFER_SIZE_PER_LINK_BYTES * (NUM_LINKS)) + (40 * (NUM_GATT_ATTRIBUTES)) + \
   (48 * (NUM_GATT_SERVICES)) + ((MEM_BLOCK_SIZE + 12) * (MBLOCKS_COUNT)) +             \
   D_LEN_EXT_SIZE(D_LEN_EXT_EN))

/**
 *
 * This macro returns the amount of memory, in bytes, needed for the storage of non-volatile data
 * for bonded devices
 *
 * @param FLASH_SEC_DB_SIZE: Size of the database used to store security information for bonded
 * devices. The recommended value for this parameter is 1024.
 * @param FLASH_SERVER_DB_SIZE: Size of the database used for service change notification for bonded
 * devices.  The recommended value for this parameter is 1024.
 * @note: the total buffer size is rounded-up to the next multiple of on flash sector size, i.e.
 * 2048 bytes
 */
...
typedef struct
{
  ...
  uint32_t  max_conn_event_length; /**< Maximum duration of the connection event when the device is
                                      in Slave mode in units of 625/256 us (~2.44 us) */
  uint16_t slave_sca;              /**< Sleep clock accuracy in Slave mode (ppm value)*/
  uint8_t  master_sca; /**< Sleep clock accuracy in Master mode (0 to 7 corresponding to 500, 250,
                          150, 100, 75, 50, 30, 20 ppm)*/
  uint8_t
           ls_source; /**< Source for the 32 kHz slow speed clock: 1: internal RO; 0: external crystal */
  uint16_t hs_startup_time; /**< Start up time of the high speed (16 or 32 MHz) crystal oscillator
...
typedef struct
{
  uint8_t* bleStartFlashAddress; /**< Start address for the non-volatile memory area allocated
                                    according to TOTAL_FLASH_BUFFER_SIZE (Aligned to 2048 bytes
                                    flash sector boundary) */
  uint32_t secDbSize;    /**< Size of the database used to store security information for bonded
                            devices (FLASH_SEC_DB_SIZE used in the calculation of
                            TOTAL_FLASH_BUFFER_SIZE). Current SUPPORTED VALUE is 1024 */
  uint32_t serverDbSize; /**< Size of the database used for service change notification for bonded
                            devices (FLASH_SERVER_DB_SIZE used in the calculation of
                            TOTAL_FLASH_BUFFER_SIZE). Current SUPPORTED VALUE is 1024 */
  uint8_t*
      stored_device_id_data_p; /**< Address of the const device id data vector (56 bytes, 32bit
                                  aligned FLASH area, all elements must be initialized to 0xFF) */
  uint8_t* bleStartRamAddress; /**< Start address of the RAM buffer for GATT database allocated
                                  according to TOTAL_BUFFER_SIZE (32bit aligned RAM area) */
  uint32_t
           total_buffer_size; /**< TOTAL_BUFFER_SIZE return value, used to check the MACRO correctness*/
  uint16_t numAttrRecord; /**< Maximum number of Attributes (i.e. the number of characteristic + the
                             number of characteristic values + the number of descriptors, excluding
                             the services) that can be stored in the GATT database. Note that
                             certain characteristics and relative descriptors are added
                             automatically during device initialization so this parameters should be
                             9 plus the number of user Attributes (NUM_GATT_ATTRIBUTES used in the
                             calculation of TOTAL_BUFFER_SIZE) */
  uint16_t numAttrServ;   /**< Maximum number of Services that can be stored in the GATT database.
                             ...
                             used in the calculation of TOTAL_BUFFER_SIZE)*/
  uint16_t attrValueArrSize; /**< Size of the storage area for Attribute values
(ATT_VALUE_ARRAY_SIZE used in the calculation of TOTAL_BUFFER_SIZE) This value depends on the number
of attributes used by application. In particular the sum of the following quantities (in octets)
...
*/
  uint8_t numOfLinks; /**< Maximum number of simultaneous connections that the device will support.
  ...
  uint8_t prWriteListSize; /**< Prepare Write List size in terms of number of packet with ATT_MTU=23
                              bytes */
  uint8_t                 mblockCount;     /**< Number of allocated memory blocks */
  uint16_t                attMtu;          /**< Maximum supported ATT_MTU size */
  hardware_config_table_t hardware_config; /**< Structure containing low level hardware
 ...
 *
 * The BTLE Stack Tick function has to be executed regularly to process incoming Link Layer packets
 and to process Host Layers procedures. All
 * stack callbacks are called by this function.
 *
 * If Low Speed Ring Oscillator is used instead of the LS Crystal oscillator this function performs
 also the LS RO calibration and hence must
 * be called at least once at every system wake-up in order to keep the 500 ppm accuracy (at least
 500 ppm accuracy is mandatory if acting as a master).
 *
 * No BTLE stack function must be called while the BTLE_StackTick is running. For example, if a BTLE
 stack function may be called inside an
 * interrupt routine, that interrupt must be disabled during the execution of BTLE_StackTick().
 Example (if a stack function may be called inside
 * UART ISR):
 * @code
 ...
 * @note The API name and parameters are subject to change in future releases.
 *
 * @param[in]  BlueNRG_Stack_Init_params_p      pointer to the const structure containing memory and
 ...
 */
tBleStatus
BlueNRG_Stack_Initialization(const BlueNRG_Stack_Initialization_t* BlueNRG_Stack_Init_params_p);
