...
#include "bluenrg_x_device.h"
  ...
  typedef struct
  {
    uint32_t DMA_PeripheralBaseAddr; /*!< Specifies the peripheral base address
                                        for DMAy Channelx. */

    uint32_t DMA_MemoryBaseAddr; /*!< Specifies the memory base address for DMAy
                                    Channelx. */

    uint32_t DMA_DIR; /*!< Specifies if the peripheral is the source or
                         destination. This parameter can be a value of @ref
                         DMA_data_transfer_direction */

    uint32_t DMA_BufferSize; /*!< Specifies the buffer size, in data unit, of
                                the specified Channel. The data unit is equal to
                                the configuration set in DMA_PeripheralDataSize
                                  or DMA_MemoryDataSize members depending in the
                                transfer direction. */

    uint32_t DMA_PeripheralInc; /*!< Specifies whether the Peripheral address
                                   register is incremented or not. This
                                   parameter can be a value of @ref
                                   DMA_peripheral_incremented_mode */

    uint32_t DMA_MemoryInc; /*!< Specifies whether the memory address register
                               is incremented or not. This parameter can be a
                               value of @ref DMA_memory_incremented_mode */
    ...
    uint32_t DMA_Mode; /*!< Specifies the operation mode of the DMAy Channelx.
                          ...
                          the memory-to-memory data transfer is configured on
                          the selected Channel */

    uint32_t DMA_Priority; /*!< Specifies the software priority for the DMAy
                              Channelx. This parameter can be a value of @ref
                              DMA_priority_level */

    uint32_t DMA_M2M; /*!< Specifies if the DMAy Channelx will be used in
                         memory-to-memory transfer. This parameter can be a
                         value of @ref DMA_memory_to_memory */
  } DMA_InitType;
  ...
  /** @defgroup DMA_Data_Transfer_Direction_Definitions   Data Transfer
...
#define IS_DMA_DIR(DIR)       (((DIR) == DMA_DIR_PeripheralDST) || ((DIR) == DMA_DIR_PeripheralSRC))
  ...
  /** @defgroup DMA_Peripheral_Incremented_Mode_Definitions   Peripheral
   * Incremented Mode Definitions
   * @{
   */
...
#define IS_DMA_PERIPHERAL_INC_STATE(STATE) \
  (((STATE) == DMA_PeripheralInc_Enable) || ((STATE) == DMA_PeripheralInc_Disable))
  /**
   * @}
   */

  /** @defgroup DMA_Memory_Incremented_Mode_Definitions   Memory Incremented
   * Mode Definitions
   * @{
   */
...
#define IS_DMA_MEMORY_INC_STATE(STATE) \
  ...
  /** @defgroup DMA_Peripheral_Data_Size_Definitions  Peripheral Data Size
   * Definitions
   * @{
   */
...
#define IS_DMA_PERIPHERAL_DATA_SIZE(SIZE)                                                    \
  (((SIZE) == DMA_PeripheralDataSize_Byte) || ((SIZE) == DMA_PeripheralDataSize_HalfWord) || \
...
#define IS_DMA_MEMORY_DATA_SIZE(SIZE)                                                \
  ...
  /** @defgroup DMA_Circular_Normal_Mode_Definitions  Circular Normal Mode
   * Definitions
   * @{
   */
...
#define IS_DMA_PRIORITY(PRIORITY)                                                \
  (((PRIORITY) == DMA_Priority_VeryHigh) || ((PRIORITY) == DMA_Priority_High) || \
...
#define IS_DMA_CONFIG_FLAG(FLAG) (((FLAG)&0x0E) != 0)
  ...
  /** @defgroup DMA_Interrupts_Channel_Definitions   Interrupts Channel
   * Definitions
   * @{
   */
...
#define IS_DMA_ADC_CH(SEL)                                                                      \
  (((SEL) == DMA_ADC_CHANNEL0) || ((SEL) == DMA_ADC_CHANNEL1) || ((SEL) == DMA_ADC_CHANNEL2) || \
   ((SEL) == DMA_ADC_CHANNEL3) || ((SEL) == DMA_ADC_CHANNEL4) || ((SEL) == DMA_ADC_CHANNEL5) || \
   ((SEL) == DMA_ADC_CHANNEL6) || ((SEL) == DMA_ADC_CHANNEL7))
