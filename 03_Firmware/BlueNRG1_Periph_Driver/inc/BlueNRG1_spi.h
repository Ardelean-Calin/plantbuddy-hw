...
#define IS_SPI_COM_MODE(MODE)                                           \
  (((MODE) == SPI_FULL_DUPLEX_MODE) || ((MODE) == SPI_TRANSMIT_MODE) || \
   ((MODE) == SPI_RECEIVE_MODE) || ((MODE) == SPI_COMBINED_MODE))
...
#define IS_SPI_FRMFRMT(FRMT) (((FRMT) == SPI_FrmFrmt_Motorola) || ((FRMT) == SPI_FrmFrmt_Microwire))
...
#define IS_SPI_CLEAR_IT(IT) (((IT) == SPI_IT_TUR) || ((IT) == SPI_IT_RT) || ((IT) == SPI_IT_ROR))

/* This macro checks if IT is a valid interrupt */
#define IS_SPI_GET_IT(IT)                                                                       \
  (((IT) == SPI_IT_TX) || ((IT) == SPI_IT_RX) || ((IT) == SPI_IT_RT) || ((IT) == SPI_IT_ROR) || \
...
#define IS_SPI_GET_FLAG(FLAG)                                                          \
  (((FLAG) == SPI_FLAG_TFE) || ((FLAG) == SPI_FLAG_TNF) || ((FLAG) == SPI_FLAG_RNE) || \
   ((FLAG) == SPI_FLAG_RFF) || ((FLAG) == SPI_FLAG_BSY))
  /**
   * @}
   */
...
#define IS_SPI_DMAREQ(DMAREQ)                                    \
  (((DMAREQ) == SPI_DMAReq_Tx) || ((DMAREQ) == SPI_DMAReq_Rx) || \
   ((DMAREQ) == (SPI_DMAReq_Rx | SPI_DMAReq_Tx)))
...
#define IS_SPI_FIFO_LEV(FIFO_LEV)                                      \
  (((FIFO_LEV) == SPI_FIFO_LEV_1) || ((FIFO_LEV) == SPI_FIFO_LEV_4) || \
...
#define IS_SPI_ENDIAN(FORMAT)                                                        \
  (((FORMAT) == SPI_ENDIAN_MSByte_MSBit) || ((FORMAT) == SPI_ENDIAN_LSByte_MSBit) || \
   ((FORMAT) == SPI_ENDIAN_MSByte_LSBit) || ((FORMAT) == SPI_ENDIAN_LSByte_LSBit))
/**
 * @}
 */

/** @defgroup SPI_Delay_Between_Frames_Definition Delay Between Frames
 * Definition
 * @{
 */
