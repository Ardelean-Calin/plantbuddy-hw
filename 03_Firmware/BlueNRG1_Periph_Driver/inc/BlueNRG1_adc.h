...
#define IS_ADC_OSR(OSR)                                                         \
  (((OSR) == ADC_OSR_200) || ((OSR) == ADC_OSR_100) || ((OSR) == ADC_OSR_64) || \
   ((OSR) == ADC_OSR_32))
/**
 * @}
 */
...
#define ADC_Input_AdcPin2                             \
  ((uint8_t)0x01) /* Single ended input from ADC2 pin \
                   */
#define ADC_Input_AdcPin1                             \
  ((uint8_t)0x02) /* Single ended input from ADC1 pin \
                   */
...
#define IS_ADC_INPUT(INPUT)                                                          \
  (((INPUT) == ADC_Input_None) || ((INPUT) == ADC_Input_AdcPin1) ||                  \
   ((INPUT) == ADC_Input_AdcPin2) || ((INPUT) == ADC_Input_AdcPin12) ||              \
   ((INPUT) == ADC_Input_TempSensor) || ((INPUT) == ADC_Input_BattSensor) ||         \
   ((INPUT) == ADC_Input_Internal0V60V6) || ((INPUT) == ADC_Input_Internal1V20V0) || \
...
/* The user must discards the first 10 converted samples in continuous mode if
 * the SKIP bit is 0. The user must discards the first 3 converted samples in
 * continuous mode if the SKIP bit is 1.  */
#define ADC_ConversionMode_Continuous ((uint8_t)0x08)

#define IS_ADC_CONVERSIONMODE(CONVERSIONMODE)         \
  (((CONVERSIONMODE) == ADC_ConversionMode_Single) || \
...
#define IS_ADC_ATTENUATION(ATTENUATION)                                                  \
  (((ATTENUATION) == ADC_Attenuation_0dB) || ((ATTENUATION) == ADC_Attenuation_6dB02) || \
   ((ATTENUATION) == ADC_Attenuation_9dB54))
  /**
   * @}
   */

  /** @defgroup ADC_Reference_Voltage_Definitions   Reference Voltage
   * Definitions
   * @{
   */
...
#define IS_ADC_REFERENCEVOLTAGE(REFERENCEVOLTAGE)      \
 ...
 * @{
 */
#define ADC_MIC_800KHZ                                                  \
  ((uint8_t)0x00) /*!< Enable the 800 kHz through IO0 for external MEMS \
                     microphone */
#define ADC_MIC_1600KHZ                                                 \
  ((uint8_t)0x01) /*!< Enable the 1.6 MHz through IO0 for external MEMS \
                     microphone */
...
#define IS_ADC_GET_FLAG(FLAG) (((FLAG)&0x0F) != 0)
  ...
  float ADC_ConvertSingleEndedVoltage(int16_t raw_value, uint8_t channel, uint8_t vRef,
                                      uint8_t attenuation);
