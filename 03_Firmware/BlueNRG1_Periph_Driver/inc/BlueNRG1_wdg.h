...
* \section WDG How tho use the Watchdog functionality?
   ...
   In order to use this functionality the best way is to add  the watchdog
reload in the main loop (and not on the WDG IRQ handler: WDG_Handler()). User
should follow these steps:

   1: Add a Watchdog initialization function to be called before entering on
main loop: (WATCHDOG_TIME is a timeout defined based on the application
scenario):
   ...
   - NOTE: The option to use the WDG_Handler() it is not mandatory at all, and
it is just an implementation choice. If user wants to use this method (not
mandatory), then he  must clear the pending bit associated to the watchdog
