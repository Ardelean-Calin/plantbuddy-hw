...
#include "bluenrg_x_device.h"
  ...
  typedef struct
  {
    uint32_t UART_BaudRate; /*!< This member configures the UART communication
                               baud rate. */

    uint8_t UART_WordLengthTransmit; /*!< Specifies the number of data bits
                                        transmitted in a frame. This parameter can
                                        be a value of @ref UART_Word_Length */

    uint8_t UART_WordLengthReceive; /*!< Specifies the number of data bits
                                       received in a frame. This parameter can
                                       be a value of @ref UART_Word_Length */
    ...
    uint8_t UART_Mode; /*!< Specifies whether the Receive or Transmit mode is
                          enabled or disabled.
                            This parameter can be a value of @ref UART_Mode */

    uint8_t UART_HardwareFlowControl; /*!< Specifies whether the hardware flow
                                         control mode is enabled or disabled.
                                           This parameter can be a value of @ref
                                         UART_Hardware_Flow_Control */

    FunctionalState UART_FifoEnable; /*!< Specifies whether the FIFO is enabled
                                        or disabled. This parameter can be a
                                        value of @ref UART_Fifo_Enable */
  } UART_InitType;
...
#define IS_UART_WORD_LENGTH(LENGTH)                                        \
  (((LENGTH) == UART_WordLength_5b) || ((LENGTH) == UART_WordLength_6b) || \
   ((LENGTH) == UART_WordLength_7b) || ((LENGTH) == UART_WordLength_8b))
  /**
   * @}
   */
...
#define IS_UART_PARITY(PARITY) \
  (((PARITY) == UART_Parity_No) || ((PARITY) == UART_Parity_Even) || ((PARITY) == UART_Parity_Odd))
  /**
   * @}
   */
...
/** @defgroup UART_Hardware_Flow_Control_Definition Hardware Flow Control
 * Definition
 * @{
 */
...
#define UART_HardwareFlowControl_RTS_CTS \
  ((uint8_t)0x03) /*!< Enables the RTS and CTS hardware flow control */
...
#define IS_UART_FLAG(FLAG)                                                                   \
  (((FLAG) == UART_FLAG_CTS) || ((FLAG) == UART_FLAG_BUSY) || ((FLAG) == UART_FLAG_RXFE) ||  \
   ((FLAG) == UART_FLAG_TXFF) || ((FLAG) == UART_FLAG_RXFF) || ((FLAG) == UART_FLAG_TXFE) || \
   ((FLAG) == UART_FLAG_OE) || ((FLAG) == UART_FLAG_FE) || ((FLAG) == UART_FLAG_PE) ||       \
   ((FLAG) == UART_FLAG_BE))

/* This macro checks if FLAG is a valid UART clearable flag */
#define IS_UART_CLEAR_FLAG(FLAG)                                                       \
  (((FLAG) == UART_FLAG_FE) || ((FLAG) == UART_FLAG_PE) || ((FLAG) == UART_FLAG_BE) || \
   ((FLAG) == UART_FLAG_OE))
...
#define IS_FIFO_LEV(FIFO_LEV)                                        \
  (((FIFO_LEV) == FIFO_LEV_1_64) || ((FIFO_LEV) == FIFO_LEV_1_32) || \
   ((FIFO_LEV) == FIFO_LEV_1_16) || ((FIFO_LEV) == FIFO_LEV_1_8) ||  \
   ((FIFO_LEV) == FIFO_LEV_1_4) || ((FIFO_LEV) == FIFO_LEV_1_2) || ((FIFO_LEV) == FIFO_LEV_3_4))

/**
 * @}
 */

/** @defgroup UART_Software_Flow_Control_Mode_Definition Software Flow Control
 * Mode Definition
 * @{
 */
...
#define SW_FLOW_CTRL_ALL_CHARS \
  ((uint8_t)3) /*!< Software flow control XON1 and XOFF1, XON2 and XOFF2 */

/* This macro checks if SW_FLOW_CTRL is a valid software flow control value */
#define IS_SW_FLOW_CTRL(SW_FLOW_CTRL)                                                    \
  (((SW_FLOW_CTRL) == NO_SW_FLOW_CTRL) || ((SW_FLOW_CTRL) == SW_FLOW_CTRL_XON1_XOFF1) || \
   ((SW_FLOW_CTRL) == SW_FLOW_CTRL_XON2_XOFF2) || ((SW_FLOW_CTRL) == SW_FLOW_CTRL_ALL_CHARS))
  /**
   * @}
   */
