 ...
 * @date    Jan-2020
 * @brief   This file contains all the functions prototypes for the radio
 *firmware library.
 ******************************************************************************
 * @attention
 *
...
#define MAX_PACKET_LENGTH \
  (38 + HEADER_LENGTH) // 31 in data channel, 38 in adv. channel (-4 if encryption)
#endif
...
#define IRQ_ERR_ENC                                                            \
  (1UL << 23) /* A packet is received, but there is an error in the MIC field. \
               */
...
/* The bit determines if the action (RX or TX) is going to be executed based on
 * the back-to-back time or based on the WakeupTime. 0: Based on the
 * back-to-back time (default 150 µs). 1: Based on the WakeupTime.
 */
#define TIMER_WAKEUP 0x04

/* This bit activates automatic channel increment. The API RADIO_SetChannel sets
 * the value of the increment. 0: No increment 1: Automatic increment
 */
#define INC_CHAN 0x10

/* It determines if the WakeupTime field of the ActionPacket is considered as
 * absolute time or relative time to the current. 0: Absolute 1: Relative
 */
#define RELATIVE 0x20

/* This bit sets where the position of time stamp is taken, the beginning of the
 * packet or the end of it. RX only. 0: End of the Packet 1: Beginning of the
 * packet
 */
#define TIMESTAMP_POSITION 0x80
  ...
  struct ActionPacket
  {
    uint8_t StateMachineNo;       /* This parameter indicates the state machine number
                                     for this action. From 0 to 7. */
    uint8_t ActionTag;            /* The configuration of the current action.
                                   * Action Tag: PLL_TRIG, TXRX, TIMER_WAKEUP, INC_CHAN,
                                   * TIMESTAMP_POSITION, RELATIVE */
    uint32_t WakeupTime;          /* Contains the wakeup time in microsecond if it is
                                   * relative.    It should not be more than 24 bits if it
                                   * is absolute.
                                   * It only applies if TIMER_WAKEUP flag is set in
                                   * ActionTag. */
    uint32_t ReceiveWindowLength; /* Sets RX window size in microsecond.
                                     Applicable only for RX actions. */
    uint8_t* data;                /* Pointer to the array with the data to send (header, length and
                                   * data field), for TX. Pointer to the array where the data
                                   * received are copied, for RX. In case of RX, the array must
                                   * have the max size MAX_PACKET_LENGTH. */
    uint32_t status;              /* The Status Register with the information on the action. */
    uint32_t timestamp_receive;   /* This field contains the timestamp when a
                                   * packet is received. Intended to be used in
                                   * the dataRoutine() callback routine. RX only.
                                   */
    int32_t         rssi;         /* The rssi of the packet was received with. RX only. */
    BlueTransStruct trans_packet; /* This is a linked list, which is going to be used by
                                   * Hardware. User does not need to do anything. */
    ActionPacket* next_true;      /* Pointer to next ActionPacket if condRoutine()
                                     returns TRUE */
    ActionPacket* next_false;     /* Pointer to next ActionPacket if condRoutine()
                                     returns FALSE */
    uint8_t (*condRoutine)(
        ActionPacket*); /* User callback that decide the next ActionPacket to use.
    ...
    uint8_t trans_config; /* This is for configuring the device for TX or RX.
