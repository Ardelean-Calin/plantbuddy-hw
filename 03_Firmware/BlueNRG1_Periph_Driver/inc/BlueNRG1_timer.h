...
#include <stdio.h>
  ...
  typedef struct
  {
    uint16_t XTAL_StartupTime; /*!< XTAL startup in 2.44 us unit */
                                ...
                                * otherwise it will be assumed fixed at 32.768 kHz.                            Ignored if
                                * periodic                            calibration is active (TIMER_PeriodicCalibration = TRUE).
                                */
    BOOL TIMER_InitialCalibration;
    BOOL TIMER_PeriodicCalibration; /*!< Assume that 32 KHz clock requires
                                       periodic calibration, i.e. varies with
                                       temperature */
  } TIMER_InitType;

  typedef struct timer_calibration_s
  {
    uint64_t last_calibration_time; /** Absolute system time when last
    ...
    uint8_t  periodic_calibration;  /** Tells whether periodic hardware
                                       calibration is needed or not */
    uint32_t last_calibration_machine_time; /** Last machine time when
                                               calibration was performed */
...
/** This define represents the marging we take for long sleep to allow the
...
/*This threshold should be the time to allow the device to go correctly in
 * sleep*/
...
#define TIMER_SYSTICK_PER_10MS   4096
  ...
  /**
   * @brief Perform a low speed clock calibration and store results in the
   * global context. It updates the XTAL startup time and the interpolator
   * configuration. It updates also the cumulative STU variable, so it should be
   * called peiodically to manage timer wrapping, regardless of actual need of
  ...
  /**
   * @brief Set the wakeup time to the specified delay. The delay is converted
   * in machine time and only 20 most significant bits are taken into account.
   * The XTAL startup time is not taken into account for the wakeup, i.e. the
   * system does not wait for the XTAL startup time parameter to trigger the
   * interrupt. The delay is translated into machine time unit (MTU) and it is
   * assigned to the wakeup register.
   * @param delay: Delay from the current time expressed in system time unit
   * (STU). Range is 0 to maximum value of STU. The maximum value STU is
   * dependent on the speed of the low speed oscillator. A value too small will
   * force the timer to wrap, so it is reccommended to use at least 5-10 STU.
  ...
  /**
   * @brief Return the system time referred to the absolute machine time passed
   * as parameter.
   * @param time: Absolute machine time in the past
   * @warning User should guarantee that call to this function are performed in
  ...
  /**
   * @brief Return the system time referred to the time in microseconds passed
   * as parameter.
  ...
  /**
   * @brief Return the machine time referred to the time in microseconds passed
   * as parameter.
   * @param time: time in microseconds
   ...
   * time to settle after exit from sleep or not. If the timeout is enough in
   * the future, a preconfigured radio transaction is going to trigger at the
   * specified timeout. The timeout is translated into machine time unit (MTU)
   * and it is assigned to the wakeup register. If it is sleeping, the CPU wakes
   * up (no IRQ) when the absolute time mathches the 20 MSB of the timeout
   *        written in the register. The transmission or reception starts when
   * the timer value matches the value of timeout plus the time the analog part
   * needs to settle. All the 24 bits are considered in this case. An IRQ
   * related to this event can be generated only when the transaction is
   * completed. In order to get a precise timeout, the radio setup delays are
   * taken into account. They differ if the event is a transmission or reception
   * and if the PLL calibration is requested or not.
   * @param timeout: Absolute time expressed in system time unit (STU).
   *        It must represent a time in the future.
   *        If timeout is less than current time, it is considered as it is in
   * the future (wrapping). The maximum value in STU depends on the speed of the
  ...
  /**
   * @brief Return the status of the Radio timers and the last value programmed
   * in the register.
   ...
   */
  __STATIC_INLINE void TIMER_SetHostTimeoutReg(uint32_t value)
  {
    BLUE_CTRL->HOST_WKUP_TIMER = value;
  }
