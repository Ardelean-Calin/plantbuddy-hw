...
#include "bluenrg_x_device.h"
  ...
  typedef struct
  {
    ...
    uint8_t MFT_Clock1; /*!< Specifies the MFT clock selection, for counter A.
                              This value can be a value of @ref
                           MFT_Clock_Selector_Definition */

    uint8_t MFT_Clock2; /*!< Specifies the MFT clock selection, for counter B.
                              This value can be a value of @ref
                           MFT_Clock_Selector_Definition */

    uint8_t MFT_Prescaler; /*!< Specifies the MFT prescaler only for
                              MFT_PRESCALED_CLK */
...
#define IS_MFT_MODE(MODE)                                                         \
  (((MODE) == MFT_MODE_1) || ((MODE) == MFT_MODE_1a) || ((MODE) == MFT_MODE_2) || \
   ((MODE) == MFT_MODE_3) || ((MODE) == MFT_MODE_4))
...
#define MFT_LOW_SPEED_CLK                                                   \
  ((uint8_t)0x04) /*!< 16 MHz clock without prescaler (only when the system \
                     clock is 32 MHz). */

#define IS_MFT_CLOCK_SEL(MODE)                                                                  \
  (((MODE) == MFT_NO_CLK) || ((MODE) == MFT_PRESCALED_CLK) || ((MODE) == MFT_EXTERNAL_EVENT) || \
   ((MODE) == MFT_PULSE_ACCUMULATE) || ((MODE) == MFT_LOW_SPEED_CLK))
...
#define IS_MFT_PULSETRAIN_TRIGGER(MODE) \
  (((MODE) == MFT_TnB_TRIGGER) || ((MODE) == MFT_SOFTWARE_TRIGGER))
