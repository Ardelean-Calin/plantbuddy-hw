 ...
 * @param  SPI_InitStruct: pointer to a @ref SPI_InitType structure which will
 * be initialized.
 * @retval None
 */
 ...
 *         @arg SPI_IT_TX: Transmit FIFO half empty or less condition interrupt
 * mask.
 *         @arg SPI_IT_RX: Receive FIFO half full or less condition interrupt
 * mask.
 *         @arg SPI_IT_RT: Receive FIFO not empty and no read prior to timeout
 * period interrupt mask.
 *         @arg SPI_IT_ROR: Receive FIFO written to while full condition
 * interrupt mask.
 ...
 *					SPI peripherial clocks have to be enabled first
 *before the use.
 * @param  None
 * @retval None
 */
void SPI_ClearRXFIFO(void)
{
  ...
  while (SET == SPI_GetFlagStatus(SPI_FLAG_RNE)) { SPI_ReceiveData(); }
}
 ...
 *					@arg SPI_COMBINED_MODE: SPI combined
 *communication mode.
 * @retval None
 */
