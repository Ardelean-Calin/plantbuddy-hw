 ...
 *          BLE stack library. In that case, the API implementation is managed
 *by the library itself. The API calls remain the same. In the future release
 *the BLE stack library will rely on the Timer module library.
 * @author  RF Application Team
 * @date    Jan-2020
 * @brief   This file provides a set of function to manage the sleep timer.
 * @details The basic support is provided by a low speed oscillator with a
 *nominal speed of 32.768 kHz. The low speed oscillator provides a counter that
 *increments at a nominal rate of 30.52 us. The counter is 24 bits wide and the
 *low speed oscillator only increments bits 23:4 (most significant 20 bits) The
 *remaining 4 bits are interpolated using the high speed 16 MHz clock. The
 *counter unit are called conventionally machine time unit (MTU). At the end,
 *the counter resolution will be about 1.91 us and it will wrap every 32 sec. It
 *is important to notice that the interpolated bits will only be functional when
 *the system is out of sleep. While in sleep only the 20 most significant bits
 *will tick. The user can setup a timer that will wakeup the system (if asleep)
 *and it will trigger an interrupt when the timer expires. The user timer will
 *not take into account the high speed XTAL wakeup time. The possible options
 *for the low speed oscillator are:
 * - external 32.768 kHz XTAL
 * - internal ring oscillator (RO)
 *
 * The external 32 kHz XTAL meets the assumption made above regarding the
 *nominal speed. The internal RO, due to technology contraints, is very
 *sensitive to temperature and so the frequency absolute value and its stability
 * have a very wide spread compared to external 32 kHz XTAL. Due to this reason,
 *the concept of calibration or compensation is introduced. The calibration
 *process involves measuring the nominal 32 kHz period using the high speed 16
 *MHz clock, once this operation is performed, the software will compensate all
 *the timing using the most recent calibration value, hardware will also use the
 *calibration value to tune the interpolator for the least significant 4 bits.
 * The calibration is started by hardware at power-on and software in all the
 *other cases. As a rule of thumb, it can be assumed that the internal RO varies
 *between 30kHz and 60 kHz and in general the lower the temperature, the higher
 *the frequency. In order to cope with this variation a concept of system time
 *unit (STU) is introduced. The STU is defined as scaled clock compared to MTU
 *in order to:
 * - handle variation of internal RO
 * - express easily timing dictated by Bluetooth Low Energy protocol.
 *
 * The STU are absolute unit of time and each unit represent 625/256 us, that is
 *about 2.44 us. The MTU, on the other hand, have a nominal value of 1.91 us,
 *and this value can be assumed constant when using external 32 kHz clock, but
 *it will vary (mainly with temperature variation) when using internal RO. As a
 *design choice, use of MTU is reserved as much as possible to the driver, while
 *STU is exposed to user.
 *
 ******************************************************************************
 * @attention
 *
...
typedef struct timer_context_s
{
  ...
  uint64_t last_calibration_time; /** Absolute system time when last calibration
                                     was performed */
  uint32_t hs_startup_time;       /** XTAL startup time in STU */
  uint8_t  wakeup_time_offset;    /** wakeup up time offset expressed in half slow
                                     period unit*/
  uint16_t period_slow;           /** Number of 16 MHz cycles in a low speed oscillator
                                     period*/
  uint16_t last_period_slow;      /** Number of 16 MHz cycles in a low speed
                                     oscillator period most recent */
  uint32_t period;                /** Number of 16 MHz clock cycles in (2*(SLOW_COUNT+1)) low
                                     speed oscillator periods */
  uint32_t freq;                  /** 2^39/period */
  uint32_t freq1;                 /** Round(((freq/64)*0x753)/256) */
  uint32_t period1;               /** Round (( ((period /256) * 0x8BCF6) + (((period % 256)*
                                     0x8BCF6)/256)) / 32) */
  ...
  uint32_t last_calibration_machine_time; /** Last machine time when calibration
                                             was performed */
  uint8_t periodic_calibration;           /** Tells whether periodic hardware calibration
                                             is needed or not, i.e. lsosc speed varies
  ...
  uint8_t  tx_cal_delay_st;               /** Delay in STU between TIMEOUT+wakeup_time_offset
                                             and first transmitted bit when cal is requested**/
  uint8_t tx_no_cal_delay_st;             /** Delay in STU between
                                             TIMEOUT+wakeup_time_offset and first
                                             transmitted bit when no cal is requested**/
  uint8_t rx_cal_delay_st;                /** Delay in STU between TIMEOUT+wakeup_time_offset
                                             and rx window start when cal is requested**/
  uint8_t rx_no_cal_delay_st;             /** Delay in STU between TIMEOUT+wakeup_time_offset
                                             and rx window start when no cal is requested**/
...
static void update_interpolator(uint16_t period_slow)
{
  ...
  radio activity. Otherwise the value of period slow will be loaded at the next
  wakeup timer timeout*/
  if ((__blue_RAM.BlueGlobVar.Config & 8U) == 0)
  {
    /* Fake wakeup to make sure that interpolator take into account period_slow
     * stored in RAM */
    ...
    while (timeout < 2) { timeout = (BLUE_CTRL->CURRENT_TIME + 2) & TIMER_MAX_VALUE; }
...
/**
 * @brief Translate STU to MTU, it is implemented using integer operation and it
 * is equivalent to: result=(time*625/256)*(frequency * 16)/1e6, where frequency
 * is the current low speed frequency
 * @param time: Time expressed in STU to be translated in STU
 * @param freq: Frequency of lsosc expressed as (2048*f)/25 where f is the lsosc
...
/**
 * @brief Translate MTU to STU, it is implemented using integer operation and it
 * is equivalent to: result=(time*25600)*period, where period is the current low
 * speed period in seconds.
 * @param time: Time expressed in STU to be translated in STU
 * @param period1: Period of lsosc expressed as (3*62500*286331)*p where p is
 * the lsosc period expressed in seconds.
 * @return System Time Units
 */
...
static uint64_t get_system_time_and_machine(TIMER_ContextType* context,
...
static void update_system_time(TIMER_ContextType* context)
{
  ...
  context->cumulative_time =
      context->last_calibration_time +
      machinetime_to_systime(
          TIME_ABSDIFF(current_machine_time, context->last_calibration_machine_time), period,
...
void TIMER_Init(TIMER_InitType* TIMER_InitStruct)
{
  ...
  if (TIMER_InitStruct->TIMER_PeriodicCalibration || TIMER_InitStruct->TIMER_InitialCalibration)
  {
    ...
    while (CKGEN_BLE->CLK32K_IT == 0)
      ;
    /* Set SLOW_COUNT to 23, that is calibrate over 24 clock periods, this
    number cannot be changed without changing all the integer maths function in
    the file	*/
  ...
  TIMER_Context.tx_cal_delay_st =
      DELAY_TX + us_to_systime((__blue_RAM.BlueGlobVar.TxDelay1) & 0x7F);
  TIMER_Context.tx_no_cal_delay_st =
      DELAY_TX_NO_CAL + us_to_systime((__blue_RAM.BlueGlobVar.TxDelay) & 0x7F);
  TIMER_Context.rx_cal_delay_st =
      DELAY_RX + us_to_systime((__blue_RAM.BlueGlobVar.RcvDelay1) & 0x7F);
  TIMER_Context.rx_no_cal_delay_st =
      DELAY_RX_NO_CAL + us_to_systime((__blue_RAM.BlueGlobVar.RcvDelay1) & 0x7F);
}

/**
 * @brief Perform a low speed clock calibration and store results in the global
 * context. It updates the XTAL startup time and the interpolator configuration.
 *        It updates also the cumulative STU variable, so it should be called
 * peiodically to manage timer wrapping, regardless of actual need of periodic
 * calibration.
 * @warning This function is not re-entrant since it updates the context
 * variable storing the system time.
 * @retval None
 */
...
/**
 * @brief Return TRUE if a calibration is on going. It relies on the value of
 * the interrupt status. Assumption is that no other modules are managing the
 * interrupt status of the calibrator.
 * @retval TRUE if calibration is running, FALSE otherwise.
 */
BOOL TIMER_IsCalibrationRunning(void)
{
  /* Normally the calibration is triggered automatically by hardware only at
     power up (or hardware reset), all other calibrations are started by
     software */
 ...
 * calibration.
 * @warning This function is not re-entrant since it updates the context
 * variable storing the system time. It should be called only in user context
 * and not in interrupt context.
 * @retval None
 */
 ...
 *        This is a counter that grows since the power up of the system and it
 * never wraps.
 * @return Current system time
 */
uint64_t TIMER_GetCurrentSysTime(void) { return get_system_time(&TIMER_Context); }

/**
 * @brief Disable Wakeup Timer, Timer1 and Timer2 enabling sleep.
 * @retval None
 */
void TIMER_ClearRadioTimerValue(void) { BLUE_CTRL->TIMEOUT = 1 << 28; }

/**
 * @brief Return the system time referred to the absolute machine time passed as
 * parameter.
 * @param time: Absolute machine time in the future
 * @warning User should guarantee that call to this function are performed in a
 * non-interruptible context.
 * @return System time value
 */
uint64_t TIMER_GetFutureSysTime(uint32_t time)
{
  uint32_t delta_systime =
      machinetime_to_systime(TIME_DIFF(time, TIMER_Context.last_machine_time),
                             TIMER_Context.period1, &TIMER_Context.roundingError);
...
void TIMER_GetCurrentCalibrationData(TIMER_CalibrationType* data)
{
  ...
  data->last_calibration_machine_time =
      TIMER_Context.last_calibration_machine_time; /** Last machine time when calibration
                                                      was performed */
}

/**
 * @brief Set the wakeup time to the specified delay. The delay is converted in
 * machine time and only 20 most significant bits are taken into account. The
 * XTAL startup time is not taken into account for the wakeup, i.e. the system
 * does not wait for the XTAL startup time parameter to trigger the interrupt.
 *        The delay is translated into machine time unit (MTU) and it is
 * assigned to the wakeup register.
 * @param delay: Delay from the current time expressed in system time unit
 * (STU). Range is 0 to maximum value of STU. The maximum value STU is dependent
 * on the speed of the low speed oscillator. A value too small will force the
 * timer to wrap, so it is reccommended to use at least 5-10 STU.
 * @param allow_sleep: Setting it to zero will prevent hardware to go to deep
 * sleep, if other code commands the system to go to deep sleep.
 * @warning This function should be called with interrupts disabled to avoid
 * programming the timer with a value in the past
...
uint32_t TIMER_SetWakeupTime(uint32_t delay, BOOL allow_sleep)
{
  ...
  /* 4 least significant bits are not taken into account. Let's round the value
   * summing 8 */
  BLUE_CTRL->HOST_WKUP_TIMER =
      ((current_time + (delay + 8)) & TIMER_MAX_VALUE) | (1 << 24) | (allow_sleep ? (1 << 28) : 0);
...
/**
 * @brief Program either Timer1 or the Wakeup Timer if the device has enough
 * time to settle after exit from sleep or not. If the timeout is enough in the
 * future, a preconfigured radio transaction is going to trigger at the
 * specified timeout. The timeout is translated into machine time unit (MTU) and
 * it is assigned to the wakeup register. If it is sleeping, the CPU wakes up
 * (no IRQ) when the absolute time mathches the 20 MSB of the timeout written in
 * the register. The transmission or reception starts when the timer value
 * matches the value of timeout plus the time the analog part needs to settle.
 * All the 24 bits are considered in this case. An IRQ related to this event can
 * be generated only when the transaction is completed. In order to get a
 * precise timeout, the radio setup delays are taken into account. They differ
 * if the event is a transmission or reception and if the PLL calibration is
 * requested or not.
 * @param timeout: Absolute time expressed in system time unit (STU).
 *        It must represent a time in the future.
 *        If timeout is less than current time, it is considered as it is in the
 * future (wrapping). The maximum value in STU depends on the speed of the low
 * speed oscillator.
 ...
 * @retval 1 if a correct timeout cannot be programmed (the timeout is too
 * close).
 */
uint8_t TIMER_SetRadioTimerValue(uint32_t timeout, BOOL event_type, BOOL cal_req)
{
  uint32_t current_time, relTimeout, delay, destination, device_delay_st;
  ...
  /* At this point, it is care of the upper layers to guarantee that the timeout
   * represents an absolute time in the future */
  ...
  {
    /*This means that wakeup_timer will be programmed with a time in the past
    and it is not possible to program it. Therefore Timer1 is selected and no
    sleep is requested */
  ...
  BLUE_CTRL->TIMEOUT =
      ((current_time + delay) & TIMER_MAX_VALUE) | destination | (enable_sleep ? (1 << 28) : 0);
...
/**
 * @brief Return the status of the Radio timers and the last value programmed in
 * the register.
 ...
 * @param nextRadioActivity: pointer to value that is going to have last TIMEOUT
 * value programmed
 * @param nextRadioActivity: pointer to value that is going to have last
 * HOST_WKUP_TIMER value programmed
 * @retval None
 */
...
/**
 * @brief Return the system time referred to the time in microseconds passed as
 * parameter.
 * @param time: time in microseconds
 * @return System time value
 */
uint32_t TIMER_UsToSystime(uint32_t time) { return us_to_systime(time); }

/**
 * @brief Return the machine time referred to the time in microseconds passed as
 * parameter.
 * @param time: time in microseconds
 * @return Machine time value
 */
uint32_t TIMER_UsToMachinetime(uint32_t time) { return us_to_machinetime(time); }

/**
 * @brief Return timer capture register value in STU.
 * @return value in STU
 */
uint64_t TIMER_GetAnchorPoint(void) { return TIMER_GetPastSysTime(BLUE_CTRL->TIMER_CAPTURE); }
