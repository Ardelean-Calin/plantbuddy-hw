...
uint16_t SysCtrl_GetWakeupResetReason(void)
{
  ...
  if ((tmpSoc == 0) && ((tmpBle & BLE_REASON_FROM_IO9) == BLE_REASON_FROM_IO9))
  {
    wakeup_src |= RESET_BLE_WAKEUP_FROM_IO9;
  }
  if ((tmpSoc == 0) && ((tmpBle & BLE_REASON_FROM_IO10) == BLE_REASON_FROM_IO10))
  {
    wakeup_src |= RESET_BLE_WAKEUP_FROM_IO10;
  }
  if ((tmpSoc == 0) && ((tmpBle & BLE_REASON_FROM_IO11) == BLE_REASON_FROM_IO11))
  {
    wakeup_src |= RESET_BLE_WAKEUP_FROM_IO11;
  }
  if ((tmpSoc == 0) && ((tmpBle & BLE_REASON_FROM_IO12) == BLE_REASON_FROM_IO12))
  {
    wakeup_src |= RESET_BLE_WAKEUP_FROM_IO12;
  }
  if ((tmpSoc == 0) && ((tmpBle & BLE_REASON_FROM_IO13) == BLE_REASON_FROM_IO13))
  {
    wakeup_src |= RESET_BLE_WAKEUP_FROM_IO13;
  }

  if ((tmpSoc == 0) && ((tmpBle & BLE_REASON_FROM_TIMER1) == BLE_REASON_FROM_TIMER1))
  {
    wakeup_src |= RESET_BLE_WAKEUP_FROM_TIMER1;
  }
  if ((tmpSoc == 0) && ((tmpBle & BLE_REASON_FROM_TIMER2) == BLE_REASON_FROM_TIMER2))
  {
    wakeup_src |= RESET_BLE_WAKEUP_FROM_TIMER2;
  }
  if ((tmpSoc == 0) && ((tmpBle & BLE_REASON_POR_RST) == BLE_REASON_POR_RST))
  {
    wakeup_src |= RESET_BLE_POR;
  }
  if ((tmpSoc == 0) && ((tmpBle & BLE_REASON_BOR_RST) == BLE_REASON_BOR_RST))
  {
 ...
 * @param  XOFreq: configure the system for the correct XO external clock used,
 * the value can be
 *         @arg XO_32MHz enable the XO divider and expect a 32 MHz external
 * clock.
 *         @arg XO_16MHz disable the XO divider and expect a 16 MHz external
 * clock.
 * @retval None
 */
