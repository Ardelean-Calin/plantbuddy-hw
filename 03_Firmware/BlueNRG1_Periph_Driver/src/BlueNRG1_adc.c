...
{
  return (ADC_KBATT *
          (ADC_ConvertSingleEndedVoltage(raw_value, ADC_Input_AdcPin2, ADC_ReferenceVoltage_0V6,
                                         ADC_Attenuation_0dB)));
}
...
{
  return (
      (ADC_KTEMP * (ADC_ConvertSingleEndedVoltage(raw_value, ADC_Input_AdcPin2,
                                                  ADC_ReferenceVoltage_0V6, ADC_Attenuation_0dB))) -
      ADC_CTEMP);
}
float ADC_ConvertTemperatureSensorFarenait(int16_t raw_value, uint8_t vRef, uint8_t attenuation)
{
  return ((722.0 * (ADC_ConvertSingleEndedVoltage(raw_value, ADC_Input_AdcPin2,
                                                  ADC_ReferenceVoltage_0V6, ADC_Attenuation_0dB))) -
          499.0);
}
 ...
 * @param  ADC_InitStruct: pointer to an @ref ADC_InitType structure which will
 * be initialized.
 * @retval None
 */
 ...
 * @param  ADC_InitStruct: pointer to an @ref ADC_InitType structure that
...
/**
 * @brief  Select the frequency of the clock signal for an external MEMS
 * microphone (through IO0)
 * @param  Frequency: the value can be
 *         @arg ADC_MIC_800KH Enable the 800 kHz through IO0 for external MEMS
 * microphone
 *         @arg ADC_MIC_1600KH Enable the 1.6 MHz through IO0 for external MEMS
 * microphone
 * @retval None
 */
...
float ADC_GetConvertedData(uint8_t DataType, uint8_t Vref)
{
  ...
  else if (DataType == ADC_Input_AdcPin12 || DataType == ADC_Input_Internal0V60V6 ||
           DataType == ADC_Input_Internal1V20V0)
  {
...
/**
 * @brief  Generates the slope and the offset used for the software
 * compensation.
 * @param  None
...
/**
 * @brief  Apply the slope and the offset in order to compensate the output
 * value. Before call this function, the API ADC_SwCalibration() must be called
 * and it must return 0 (success). value can be an ADC value from battery sensor
 * or ADC pins.
