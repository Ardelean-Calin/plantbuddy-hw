 ...
 * @{
 */
#define DISABLE_INTERRUPT(irqNum) \
  {                               \
    NVIC_DisableIRQ(irqNum);      \
  }
#define ENABLE_INTERRUPT(irqNum) \
  {                              \
...
static volatile BlueGlobPerMaster* const bluedata =
    __blue_RAM.BlueGlobPerMasterVar; // (BlueGlobPerMaster*) (BLUEGLOB_BASE+12);
...
static int32_t read_RSSI(void)
{
  ...
  int rssi_int16 =
      ((globalParameters.rssiLevel[0] >> 16) & 0xff00) | (globalParameters.rssiLevel[1] & 0xff);
...
static void write_radio_config(char* ptr)
{
  ...
  int i                   = 0;
  while ((BLUE_CTRL->RADIO_CONFIG & 0x10000) != 0) { i = i + 1; }
}
...
void RADIO_IRQHandler(void)
{
  ...
  if ((int_value & IRQ_DONE) != 0)
  {
    ...
    if ((globalParameters.current_action_packet->condRoutine(
            globalParameters.current_action_packet)) == TRUE)
    {
    ...
    else
    {
      ...
      (bluedata + next->StateMachineNo)->remap_chan =
          (bluedata + next->StateMachineNo)->remap_chan | 0x80;
#endif
      ...
      if ((next->ActionTag & TIMER_WAKEUP) != 0)
      {
        ...
        if ((next->ActionTag & RELATIVE) != 0)
        {
          ...
          HAL_VTimer_SetRadioTimerValue(time, (next->ActionTag & TXRX),
                                        (next->ActionTag & PLL_TRIG));
        }
        else
        {
          HAL_VTimer_SetRadioTimerValue(next->WakeupTime, (next->ActionTag & TXRX),
                                        (next->ActionTag & PLL_TRIG));
        }
      }
      /* back to back */
...
/**
 * @brief  This routine force the radio to be stopped. After calling this
 * function, the ISR is not triggered unless, "MakeActionPacketPending" API
 * called again.
 * @param  None
 * @retval TRUE
 */
...
/**
 * @brief  This routine sets the 40-bit receive and transmit packet count, to be
 * used in encryption calculation. Both are pointers to char, and set the 39-bit
 * counter + 1-bit MSB as defined in the Bluetooth Low Energy spec. One is for
 * transmit packets, the other is for receive packets.
 * @param  StateMachineNo: state machine number in multi state. From 0 to 7.
 * @param  count_tx: 40-bit transmit packet count, to be used in encryption
 * nounce calculation.
 * @param  count_rcv: 40-bit receive packet count, to be used in encryption
 * nounce calculation.
 * @retval None
 */
...
/**
 * @brief  This routines sets the 8-byte encryption initial vector, and the
 * 16-byte encryption key. The definition of the encryption key and its initial
 * value is according the Bluetooth Low Energy spec.
 * @param  StateMachineNo: state machine number in multi state. From 0 to 7.
...
void RADIO_SetEncryptionAttributes(uint8_t StateMachineNo, uint8_t* enc_iv, uint8_t* enc_key)
{
  ...
  for (i = 8; i < 16; i++) { (bluedata + StateMachineNo)->enc_key[i] = enc_key[i]; }

  return;
}

/**
 * @brief  This routine sets the channel map. If channel map is not in use, do
 * not use it. The chan_remap constant is a 37-bit vector, one bit per channel
 * from 0 to 36, with the LSB corresponding to the channel 0. If the
 * corresponding bit is set, the channel is in use; if it is cleared, there is
 * an automatic remap to another channel, as defined in the Bluetooth Low Energy
 * specification. Setting all bits of the array to one, disables the channel
 * remapping, and this is the mode that should be used if the Bluetooth channel
 * remap is not in use.
 *
 * @param  StateMachineNo: state machine number in multi state. From 0 to 7.
 * @param  chan_remap: a 37-bit vector, one bit per channel from 0 to 36, with
 * the LSB corresponding to the channel 0
 * @retval None
 */
void RADIO_SetChannelMap(uint8_t StateMachineNo, uint8_t* chan_remap)
{
  ...
  for (uint8_t i = 0; i < 5; i++) { (bluedata + StateMachineNo)->chan_remap[i] = chan_remap[i]; }
  return;
}

/**
 * @brief  This routine sets the channel and the channel increment. Channel and
 * channel increment follows channel mapping corresponding to Bluetooth
 * specification. RF channel 0:  2402 MHz -> channel 37 RF channel 1:  2404 MHz
 * -> channel 0 RF channel 2:  2406 MHz -> channel 1
 *         ...
 ...
 *            Valid values for packet length for these channels are from 6
 * to 37.
 * @param  StateMachineNo: state machine number in multi state. From 0 to 7.
 ...
 *         The user shall ensure that the NetworkID meets the following
 * requirements:
 ...
 *         This value must be 0x555555 when channel frequency is one of these
 * values: 37, 38, 39.
 * @param  sca: slow clock accuracy. Period duration of the 32 kHz clock (slow
 * clock). "period_slow" defines the number of 16 MHz clock cycle in a 32 kHz
 * period. Note: the number 16 MHz clock cycle in a 32 kHz period can be
 * measured by the clockgen block. Note: If the application uses a CLK32K
 * crystal then the value is a constant provided by the firmware. (488 =
 * 16,000,000 / 32768) Note: If the application uses the 32 kHz RC clock, then
 * its value is measured in the clock_gen. Note: not used here. This value
 * should be set automatically in RADIO_CrystalCheck routine.
 * @retval None
 */
 ...
 * @param  back_to_back_time: time between two packets if wakeupTimer is not
 * used. Resolution is 1 us.
 * @retval None
 */
...
void RADIO_SetTxPower(uint8_t PowerLevel)
{
  ...
  PowerLevel <<= 3;
  for (int n = 0; n < STATEMACHINE_COUNT; n++) { (bluedata + n)->conf_byte5 = PowerLevel; }
  return;
}

/**
 * @brief  This routine turns encrypt ON and OFF. When the encryption is
 * enabled, the hardware will add 4 bytes at the end of the packet as MIC
 * (Message Authentication Code). So the user need to add 4 byte to the length
 * of the packet. The parameters enable the encryption for both. There are two
 * separated parameters for this function just compliance with previous version
 * of the driver.
 * @param  StateMachineNo: state machine number in multi state. From 0 to 7.
 * @param  EncryptFlagTx: DISABLE: encryption is turned OFF for both TX and RX
 * operations. ENABLE: encryption is turned OFF for both TX and RX operations.
 *         This parameter can be: ENABLE or DISABLE.
 * @param  EncryptFlagRcv: encryption is turned OFF for both TX and RX
 * operations. ENABLE: encryption is turned OFF for both TX and RX operations.
 *          This parameter can be: ENABLE or DISABLE.
 * @retval None
 */
void RADIO_SetEncryptFlags(uint8_t StateMachineNo, FunctionalState EncryptFlagTx,
...
/**
 * @brief  This routine should be called after writing/modifying an action
 * packet, and before it is executed via either the API mechanism, or the next
 * mechanism.
...
{
  // p->WakeupTime = (uint32_t) (p->WakeupTime * 0.525); /* convert us to
  // appropriated value.  1000 * 1/512000 = 1953.125 */
  ...
  /* p->trans_packet.rcvlen[2] = p->trans_packet.rcvlen[2] | 0x30 | 0x40 |
   * TIMESTAMP_POSITION; */
  ...
  p->trans_packet.timeout[2] =
      (globalParameters.back2backTime >> 16) & 0x0000000F; /* only 20 bits */

  p->trans_packet.byte10 = 0;
  p->trans_packet.byte10 =
      (p->ActionTag & INC_CHAN) | (p->ActionTag & PLL_TRIG) | (p->ActionTag & TXRX);
...
/**
 * @brief  This routine should be called for the first actionPacket that needs
 * to be scheduled on the radio. For it to work OK, the WakeupTime should be
 * valid, and the TIMER_WAKEUP flag set. Subsequent packets can be dispatched
 * using the same process, or by providing non-NULL pointers to the next_true
 * and next_false pointers of the ActionPacket.
 *
 * @param  p: pointer to action packet.
...
uint8_t RADIO_MakeActionPacketPending(ActionPacket* p)
{
  ...
  if (RADIO_GetStatus(&dummyTime) == BLUE_IDLE_0)
  {
    ...
    (bluedata + statemachineNo)->stat_config =
        p->trans_config; /* Transmision or reception for first packet */
    ...
    if ((p->ActionTag & RELATIVE) != 0)
    {
      ...
      returnValue =
          HAL_VTimer_SetRadioTimerValue(time, (p->ActionTag & TXRX), (p->ActionTag & PLL_TRIG));
    }
    else
    {
      returnValue = HAL_VTimer_SetRadioTimerValue(p->WakeupTime, (p->ActionTag & TXRX),
 ...
 * @param  hs_startup_time: start up time of the high speed (16 or 32 MHz)
 * crystal oscillator in units of 625/256 us (~2.44 us).
 * @param  low_speed_osc: source for the 32 kHz slow speed clock: 1: internal
 * RO; 0: external crystal.
 * @param  hot_table: reserved for future use, set to NULL.
 * @param  whitening: DISABLE: disable whitening for TX and RX. ENABLE: enable
 * whitening for TX and RX. This parameter can be: ENABLE or DISABLE.
 * @retval None
 */
...
/**
 * @brief  This routine returns the status of the Radio whether it is busy or
 * not. If it is busy, the time argument will be filled with absolute time value
 * of next radio activity. User can use this value (in comparison with current
 * time) to check that how long it is far to force the device to sleep or not.
 ...
 *          - 1 : BLUE_BUSY_NOWAKEUP_T1. Radio is busy, but there is no wakeup
 * timer on schedule but Timer1 is.
 *          - 2 : BLUE_BUSY_NOWAKEUP_T2. Radio is busy, but there is no wakeup
 * timer on schedule but Timer2 is.
 *          - 3 : BLUE_BUSY_WAKEUP. Radio is busy and wakeup timer is on the
 * schedule.
 */
...
void RADIO_StartTone(uint8_t RF_Channel, uint8_t powerLevel)
{
  ...
  {
    /* [EM:] Calculate the synt divide factor for 16 MHz quarts and +250 kHz
     * offset wrt the channel center frequency Algorithmically MAK =
     * F_rf*(2^20)/16 = F_rf*(2^16) With F_rf = (2402+2*RF_Channel)+0.25 MHz K
     * corresponds to b19-b0  of MAK A corresponds to b22-b20 of MAK M
     * corresponds to b27-b23 of MAK
     */
...
/**
 * @brief  Encrypts plain text data using AES encryption with given input key
...
void RADIO_EncryptPlainData(uint8_t* Key, uint8_t* plainData, uint8_t* cypherData)
{
  ...
  {
    key[i] = ((uint32_t)Key[i * 4]) | ((uint32_t)Key[i * 4 + 1]) << 8 |
             ((uint32_t)Key[i * 4 + 2]) << 16 | ((uint32_t)Key[i * 4 + 3]) << 24;
    cleartext[i] = ((uint32_t)plainData[i * 4]) | ((uint32_t)plainData[i * 4 + 1]) << 8 |
                   ((uint32_t)plainData[i * 4 + 2]) << 16 | ((uint32_t)plainData[i * 4 + 3]) << 24;
  }
  ...
  ii = 0;
  while ((BLUE_CTRL->STATUS & BIT_AES_READY) == 0 && ii < 100) { ii++; }
