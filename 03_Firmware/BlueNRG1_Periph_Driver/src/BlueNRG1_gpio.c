...
void GPIO_Init(GPIO_InitType* GPIO_InitStruct)
{
  ...
  assert_param(((GPIO_InitStruct->GPIO_Pin & GPIO_Pin_12) != GPIO_Pin_12) ||
               (GPIO_InitStruct->GPIO_Mode != GPIO_Output));
  assert_param(((GPIO_InitStruct->GPIO_Pin & GPIO_Pin_13) != GPIO_Pin_13) ||
               (GPIO_InitStruct->GPIO_Mode != GPIO_Output));
 ...
 * @param  GPIO_InitStruct : pointer to a @ref GPIO_InitType structure which
 * will be initialized.
 * @retval None
 */
 ...
 *         parameters in the GPIO_InitStruct. This setting is applied during low
 * power modes. This feature is applicable only for BlueNRG-2.
 * @param  GPIO_InitStruct: pointer to a @ref GPIO_InitType structure that
 *         contains the configuration information for the specified GPIO
 * peripheral.
 * @retval None
...
/**
 * @brief  Set the output state of the GIO9, GIO10, GIO11 during low power
 * modes. Only for BlueNRG-2 device.
 * @param  GPIO_Pin: specifies the GPIO pin pins to be retained
 *         This parameter can be GPIO_Pin_x where x can be (9, 10, 11) @ref
 * GPIO_Pins_Definition.
 * @retval None
 */
void GPIO_WriteBitsLowPowerModes(uint32_t GPIO_Pins, BitAction BitVal)
{
#ifdef BLUENRG2_DEVICE
  /* Check the parameters */
  ...
  /* Reset GPIO init structure parameters values */
  if ((GPIO_Pins & GPIO_Pin_9) == GPIO_Pin_9 || (GPIO_Pins & GPIO_Pin_10) == GPIO_Pin_10 ||
      (GPIO_Pins & GPIO_Pin_11) == GPIO_Pin_11)
  {
    if (BitVal == Bit_SET)
    {
      SET_BIT(SYSTEM_CTRL->SLEEPIO_OUT, (GPIO_Pins >> 9));
    }
    else
    {
      CLEAR_BIT(SYSTEM_CTRL->SLEEPIO_OUT, (GPIO_Pins >> 9));
    }
  }
#endif
}
 ...
 *         This parameter can be GPIO_Pin_x where x can be (0..25) @ref
 * GPIO_Pins_Definition. QFN32 package: 15 pins, WLCSP32 package: 14 pins, QFN48
 * package: 26 pins. More than one pin can be specified in OR logic, i.e.
 * (GPIO_Pin_0 | GPIO_Pin_1).
 * @retval None
 */
void GPIO_ToggleBitsLowPowerModes(uint32_t GPIO_Pins)
{
#ifdef BLUENRG2_DEVICE
  /* Check the parameters */
  assert_param(IS_GPIO_PIN(GPIO_Pins));
  ...
  if ((GPIO_Pins & GPIO_Pin_9) == GPIO_Pin_9 || (GPIO_Pins & GPIO_Pin_10) == GPIO_Pin_10 ||
      (GPIO_Pins & GPIO_Pin_11) == GPIO_Pin_11)
  {
    SYSTEM_CTRL->SLEEPIO_OUT ^= (GPIO_Pins >> 9);
  }
#endif
}
 ...
 * @param  GPIO_EXTIInitStruct : pointer to a @ref GPIO_EXTIConfigType
 * structure.
 * @retval None
 */
