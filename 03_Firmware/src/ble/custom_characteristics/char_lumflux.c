...
/**
 * @brief Function for adding the luminous flux characteristic.
 *
...
uint32_t char_lumflux_add_to_service(ble_cus_pb_t* p_cus)
{
    ...
    static char*             user_desc_text  = "Luminous Flux";
    luminous_flux_t          initial_ena_val = 0;

    /* Add the Luminous Flux characteristic */
    memset(&add_char_params, 0, sizeof(add_char_params));
    add_char_params.uuid              = LUM_FLUX_CHAR_UUID;
    add_char_params.uuid_type         = p_cus->uuid_type;
    add_char_params.init_len          = sizeof(luminous_flux_t);
    add_char_params.p_init_value      = (uint8_t*)&initial_ena_val;
    add_char_params.max_len           = sizeof(luminous_flux_t);
...
/**
 * @brief Function for updating the luminous flux characteristic.
 *
 * @param[in]   p_cus       PlantBuddy Service structure.
 * @param[in]   new_lflux   New value of the luminous flux.
 *
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
uint32_t char_lumflux_update(ble_cus_pb_t* p_cus, luminous_flux_t new_lflux)
{
    ...
    gatts_value.len     = sizeof(luminous_flux_t);
    gatts_value.offset  = 0;
    gatts_value.p_value = (uint8_t*)&new_lflux;

    // Update database.
    err_code = sd_ble_gatts_value_set(p_cus->conn_handle, p_cus->char_lumflux_handle.value_handle, &gatts_value);
    ...
    if ((p_cus->conn_handle != BLE_CONN_HANDLE_INVALID))
    {
        ...
        hvx_params.handle = p_cus->char_lumflux_handle.value_handle;
