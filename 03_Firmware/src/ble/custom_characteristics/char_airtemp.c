...
/**
 * @brief Function for adding the air temperature characteristic.
 *
...
uint32_t char_airtemp_add_to_service(ble_cus_pb_t* p_cus)
{
    ...
    static char*             user_desc_text  = "Air temperature";
    airtemp_t                initial_ena_val = 0;

    /* Add the air temperature characteristic */
    memset(&add_char_params, 0, sizeof(add_char_params));
    add_char_params.uuid              = AIRTEMP_CHAR_UUID;
    add_char_params.uuid_type         = p_cus->uuid_type;
    add_char_params.init_len          = sizeof(airtemp_t);
    add_char_params.p_init_value      = (uint8_t*)&initial_ena_val;
    add_char_params.max_len           = sizeof(airtemp_t);
    ...
    err_code = characteristic_add(p_cus->service_handle, &add_char_params, &(p_cus->char_airtemp_handle));
...
/**
 * @brief Function for updating the air temperature characteristic.
 *
 * @param[in]   p_cus         PlantBuddy Service structure.
 * @param[in]   new_airtemp   New value of the air temperature.
 *
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
uint32_t char_airtemp_update(ble_cus_pb_t* p_cus, airtemp_t new_airtemp)
{
    ...
    gatts_value.len     = sizeof(airtemp_t);
    gatts_value.offset  = 0;
    gatts_value.p_value = (uint8_t*)&new_airtemp;

    // Update database.
    err_code = sd_ble_gatts_value_set(p_cus->conn_handle, p_cus->char_airtemp_handle.value_handle, &gatts_value);
    ...
    if ((p_cus->conn_handle != BLE_CONN_HANDLE_INVALID))
    {
        ...
        hvx_params.handle = p_cus->char_airtemp_handle.value_handle;
