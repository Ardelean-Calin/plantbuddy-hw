 ...
 *
 * NOTE: If prepare write requests are used for a characteristic (reliable write on multiple
 * characteristics), then this value should be set to the number of prepare write needed by the
 * application.
 *
 *  [New parameter added on BLE stack v2.x]
 */
...
#define OPT_MBLOCKS                                                            \
  (6) /* 6:  for reaching the max throughput: ~220kbps (same as BLE stack 1.x) \
       */
 ...
 */
NO_INIT(uint32_t dyn_alloc_a[TOTAL_BUFFER_SIZE(NUM_LINKS, NUM_GATT_ATTRIBUTES, NUM_GATT_SERVICES,
                                               ATT_VALUE_ARRAY_SIZE, MBLOCKS_COUNT,
...
NO_INIT_SECTION(
    uint32_t
        stacklib_flash_data[TOTAL_FLASH_BUFFER_SIZE(FLASH_SEC_DB_SIZE, FLASH_SERVER_DB_SIZE) >> 2],
    ".noinit.stacklib_flash_data");

/* FLASH reserved to store: security root keys, static random address, public address */
NO_INIT_SECTION(uint8_t stacklib_stored_device_id_data[56],
                ".noinit.stacklib_stored_device_id_data");
...
#define CONFIG_TABLE                                                                      \
  {                                                                                       \
    (uint32_t*)hot_table_radio_config, MAX_CONN_EVENT_LENGTH, SLAVE_SLEEP_CLOCK_ACCURACY, \
        MASTER_SLEEP_CLOCK_ACCURACY, LOW_SPEED_SOURCE, HS_STARTUP_TIME                    \
  }
...
const BlueNRG_Stack_Initialization_t BlueNRG_Stack_Init_params = {
    ...
    (uint8_t*)dyn_alloc_a,
    TOTAL_BUFFER_SIZE(NUM_LINKS, NUM_GATT_ATTRIBUTES, NUM_GATT_SERVICES, ATT_VALUE_ARRAY_SIZE,
