 ...
 * @details This file implements the software layer that provides the
 *virtualization of the resources of a single hardware timer in order to
 *allocate many user virtual timers. Each instance of a virtual timer is placed
 *in an queue ordered by the expiration time and it can be linked to a callback.
 * The timer tick is in charge to execute the callback linked to each virtual
 *timer and to update the hardware timeout to gurantee the expiration of the
 *next virtual timer in the queue. A special virtual timer called
 *calibration/anti-wrapping timer is automatically armed by the software. This
 *timer can address two tasks:
 * - it is in charge to maintain the never wrapping virtual time base.
 * - if the slow clock calibration is enabled, it starts the calibration
 *procedure at each calibration interval specified during the initialization.
 *
 * A timer is intended as an event programmed in the future at a certain
 *absolute expiration time on a time base. In this implementation the time base
 *grows on 64 bits. Then, it never wraps. However, due to hardware timer finite
 *length and in order to maintain the timing coherency, the time base must be
 *maintained at least one time before the hardware timer wraps. Then even if the
 *slow clock calibration is disabled, the calibration/anti-wrapping timer is
 *always active with the only role to maintain the time base and it will expire
 * at a rate that depends on the hardware timer capability.
 * The time base unit is a STU that is the unit exposed to the user and it is
 *equal to 625/256 us. The calibration/anti-wrapping mechanism is not managed by
 *the user.
 *
 ...
 * This layer tries to exploit the last calibration values to program the radio
 *activity in order to improve the accuracy. In this case, the radio event is
 *not immediately programmed when it is requested, but only when the next
 *calibration values are available. Since the calibration values are available
 *inside the timer tick when the calibration is over, the application must
 *ensure that the timer tick is called after the calibration timer expiration
 *within a certain margin in order to avoid that the radio event is shifted in
 *the past and cannot be anymore programmed.
 *
 ******************************************************************************
 * @attention
 *
...
#define INCREMENT_EXPIRE_COUNT_ISR                                                 \
  (HAL_VTIMER_Context.expired_count =                                              \
       ((HAL_VTIMER_Context.expired_count + 1) == HAL_VTIMER_Context.served_count) \
           ? HAL_VTIMER_Context.expired_count                                      \
           : (HAL_VTIMER_Context.expired_count + 1))
#define INCREMENT_EXPIRE_COUNT \
  ATOMIC_SECTION_BEGIN();      \
  INCREMENT_EXPIRE_COUNT_ISR;  \
  ATOMIC_SECTION_END();

/*Minimum programmable value in sys. unit for a radio activity to be in the
 * future*/
...
{
  uint32_t PeriodicCalibrationInterval; /*!< Periodic calibration interval in
                                           ms, to disable set to 0 */
  BOOL calibration_in_progress;         /*!< Flag to indicate that a periodic
...
static VTIMER_HandleType* _remove_timer_in_queue(VTIMER_HandleType* rootNode,
                                                 VTIMER_HandleType* handle)
{
...
static VTIMER_HandleType* _insert_timer_in_queue(VTIMER_HandleType* rootNode,
                                                 VTIMER_HandleType* handle)
{
  VTIMER_HandleType* current     = rootNode;
  VTIMER_HandleType* prev        = NULL;
  ...
  *expired = 0;
  while (curr != NULL)
  {
    if (curr->active)
    {
      ...
      if (delay > 0)
      {
        /* Protection against interrupt must be used to avoid that the called
          function will be interrupted and so the timer programming will happen
          after the target time is already passed leading to a timer expiring
          after timer wraps, instead of the expected delay */
...
/* Check the number of expired timer from rootNode (ordered list of timers) and
 * return the list of expired timers */
static VTIMER_HandleType* _check_callbacks(VTIMER_HandleType*  rootNode,
                                           VTIMER_HandleType** expiredList)
{
  VTIMER_HandleType* curr        = rootNode;
  VTIMER_HandleType* prev        = NULL;
  VTIMER_HandleType* returnValue = rootNode;
...
static int _start_timer(VTIMER_HandleType* timerHandle, uint64_t time)
{
  ...
  if (_insert_timer_in_queue(HAL_VTIMER_Context.rootNode, timerHandle) == timerHandle)
  {
    ...
    if (expired)
    {
      /* A new root timer is already expired, mimic timer expire that is
      normally signaled through the interrupt handler that increase the number
      of expired timers*/
...
1) The radio event is before the next calibration. Then the timer must be
programmed. 2) The calibration timer expired but the calibration didn't start
and the values not updated as expected. Then the last calibration time is
misleading. The radio timer must be programmed. 3) The radio event is after the
next calibration event. Then the timer will be programmed with the latest
values. The check on the last calibration time is made even though the
calibration is not needed in order to avoid counter wrapping with timeouts far
in the future.
*/
void _check_radio_activity(VTIMER_RadioHandleType* timerHandle, uint8_t* expired)
{
  ...
  *expired = 0;
  ...
  {
    nextCalibrationEvent =
        calibrationData.last_calibration_time + HAL_VTIMER_Context.PeriodicCalibrationInterval;
        ...
        (currentTime > (nextCalibrationEvent + CALIB_SAFE_THR)))
    {
      ...
      {
        *expired = TIMER_SetRadioTimerValue(timerHandle->expiryTime, timerHandle->event_type,
                                            timerHandle->cal_req);
        ATOMIC_SECTION_END();
 ...
 *        The timeout of the last radio timer activity taken into account by the
 * Timer Module is saved in the variable passed as parameter.
 * @param time: Pointer of the variable where the time of the last radio
 * activity scheduled is stored The time is expressed in STU.
 ...
 *        Furthermore, it returns if the timeout is too close with respect the
 * current time and the radio activity might not be cleared in time.
...
uint8_t HAL_VTimer_ClearRadioTimerValue(void)
{
  ...
  ATOMIC_SECTION_BEGIN();
  time_diff = radioTimer.expiryTime - TIMER_GetCurrentSysTime() -
 ...
 * @retval None
 */
void HAL_VTimer_StopRadioTimer2(void) { TIMER_ClearRadioTimerValue(); }
 ...
 * @retval None
 */
void HAL_VTIMER_RadioTimerIsr(void) { radioTimer.active = FALSE; }
...
/**
 * @brief Starts a one-shot virtual timer for the given relative timeout value
 * expressed in ms
 * @param timerHandle: The virtual timer
...
int HAL_VTimerStart_ms(VTIMER_HandleType* timerHandle, uint32_t msRelTimeout)
{
  ...
  uint8_t  retVal;
  retVal =
      _start_timer(timerHandle, TIMER_GetCurrentSysTime() + (temp * TIMER_SYSTICK_PER_10MS) / 10);
  _virtualTimeBaseEnable(ENABLE);
  return retVal;
}

/**
 * @brief Schedules a radio activity for the given absolute timeout value
 * expressed in STU If the calibration of the low speed oscillator is needed, if
 * it is possible, the radio timer will be programmed with the latest
 * calibration data.
 * @param time: Absolute time expressed in STU.
 ...
 * @retval 1 if radio activity has been rejected (it is too close or in the
 * past).
 */
uint8_t HAL_VTimer_SetRadioTimerValue(uint32_t time, uint8_t event_type, uint8_t cal_req)
{
  ...
  radioTimer.expiryTime = calibrationData.last_calibration_time +
                          (uint32_t)(time - (uint32_t)calibrationData.last_calibration_time);
  radioTimer.active  = FALSE;
...
/**
 * @brief This function return the current reference time expressed in system
 * time units. The returned value can be used as absolute time parameter where
 * needed in the other HAL_VTimer* APIs
 * @return absolute current time expressed in system time units.
 */
uint32_t HAL_VTimerGetCurrentTime_sysT32(void) { return ((uint32_t)TIMER_GetCurrentSysTime()); }

/**
 * @brief Get the last anchorPoint in system time unit.
 * @return TimerCapture register in system time unit.
 */
uint64_t HAL_VTIMER_GetAnchorPoint(void) { return TIMER_GetAnchorPoint(); }

/**
 * @brief This function returns the sum of an absolute time and a signed
 * relative time.
...
/**
 * @brief Returns the absolute expiry time of a running timer expressed in
 * internal time units.
 * @param timerHandle: The virtual timer
 * @param sysTime: Absolute time expressed in internal time units.
 */
...
void HAL_VTIMER_Init(HAL_VTIMER_InitType* HAL_TIMER_InitStruct)
{
  ...
  HAL_VTIMER_Context.PeriodicCalibrationInterval =
      (TIMER_SYSTICK_PER_10MS * HAL_TIMER_InitStruct->PeriodicCalibrationInterval) / 10;
  ...
  if (HAL_VTIMER_Context.PeriodicCalibrationInterval == 0)
    ...
    HAL_VTIMER_Context.PeriodicCalibrationInterval =
        MIN(HAL_VTIMER_Context.PeriodicCalibrationInterval,
            TIMER_MachineTimeToSysTime(TIMER_MAX_VALUE - TIMER_WRAPPING_MARGIN));
  calibrationTimer.callback = calibration_callback;
  calibrationTimer.userData = NULL;
  _start_timer(&calibrationTimer,
               TIMER_GetCurrentSysTime() + HAL_VTIMER_Context.PeriodicCalibrationInterval);
}
...
void HAL_VTIMER_Tick(void)
{
  ...
  /* if there is a periodic calibration, start it in advance during the active
   * phase */
  else
  {
    if (calibrationData.periodic_calibration)
    {
      if (TIMER_GetCurrentSysTime() >
          calibrationData.last_calibration_time + TIMER_SYSTICK_PER_SECOND)
      {
        HAL_VTimer_Stop(&calibrationTimer);
        calibration_callback(&calibrationTimer);
      }
...
SleepModes HAL_VTIMER_TimerSleepCheck(SleepModes sleepMode)
{
  ...
  {
    if ((HAL_VTIMER_Context.rootNode->next == NULL) &&
        (HAL_VTIMER_Context.rootNode == &calibrationTimer))
    {
 ...
 * @retval TRUE if all vtimers have been served and the calibration procedure
 * has already finished.
 * @retval FALSE if the vtimer Tick is still busy.
 */
uint8_t HAL_VTIMER_SleepCheck(void)
{
  return ((HAL_VTIMER_Context.expired_count == HAL_VTIMER_Context.served_count) &&
          (HAL_VTIMER_Context.calibration_in_progress == FALSE));
}
