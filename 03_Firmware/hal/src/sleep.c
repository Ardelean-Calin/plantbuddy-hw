...
WEAK_FUNCTION(SleepModes App_SleepMode_Check(SleepModes sleepMode)) { return SLEEPMODE_NOTIMER; }
WEAK_FUNCTION(SleepModes BlueNRG_Stack_Perform_Deep_Sleep_Check(void)) { return SLEEPMODE_NOTIMER; }
...
/* Function to evaluate if the application has enough time to switch the
 * frequency */
static void frequency_switch(void)
{
  ...
  /* If the BLE radio is NOT active or the Wakeup comes from radio transaction,
   * the frequency switch is safe */
  if (!((reset_reason & RESET_BLE_WAKEUP_FROM_TIMER1) ||
        ((__blue_RAM.BlueGlobVar.Config & 8U) == 0)))
  { //    ((*(volatile uint32_t*)0x200000C0 & 8U) == 0U))) {
...
static void BlueNRG_InternalSleep(SleepModes sleepMode, uint8_t gpioWakeBitMask)
{
  ...
  /* Variables used to store system peripheral registers in order to restore the
   state after exit from sleep mode */
  ...
  CTXT uint32_t GPIO_DATA_saved, GPIO_OEN_saved, GPIO_PE_saved, GPIO_DS_saved, GPIO_IS_saved,
      GPIO_IBE_saved;
  CTXT uint32_t GPIO_IEV_saved, GPIO_IE_saved, GPIO_MODE0_saved, GPIO_MODE1_saved,
  ...
  CTXT uint32_t ADC_CTRL_saved, ADC_CONF_saved, ADC_IRQMASK_saved, ADC_OFFSET_LSB_saved,
      ADC_OFFSET_MSB_saved;
  ...
  for (i = 0; i < 8; i++) { NVIC_IPR_saved[i] = NVIC->IP[i]; }
  ...
  // from the application the BlueNRG-1 enters in deep sleep without make a
  // context save. So, exiting from the deep sleep the context is restored with
  // wrong random value.
  ...
  if (!wakeupFromSleepFlag)
  {
    ...
    { // At this stage the Blue Control Interrupt shall not be pending.
      ...
      if ((savedSHCSR != SCB->SHCSR) || // Verified if a SVCall Interrupt is pending
          ((savedNVIC_ISPR != NVIC->ISPR[0]) &&
           (nvicPendingMask & NVIC->ISER[0])) || // Verified if a NVIC Interrupt is pending
          ((savedICSR & 0x10000000) !=
           (SCB->ICSR & 0x10000000)) || // Verified if a PendSV interrupt is pending
          (((savedICSR & 0x4000000) != (SCB->ICSR & 0x4000000)) && (SysTick->CTRL & 0x02)))
      { // Verified if a SysTick interrupt is pending
        ...
        { // Check if the counter are wrapping
          while ((savedCurrentTime + 3) > ((*(volatile uint32_t*)BLUE_CURRENT_TIME_REG) >> 4))
            ; // Not Wrap
        }
        else
        {
          while (((*(volatile uint32_t*)BLUE_CURRENT_TIME_REG) >> 4) !=
                 (savedCurrentTime + 3 - 0xFFFFF))
            ; // Wrap
        }
      }
    }
  ...
  else
  {
    ...
    // Issue with Atollic compiler
    //    memcpy((void *)NVIC->IP, (void*)NVIC_IPR_saved,
    //    sizeof(NVIC_IPR_saved));
    for (i = 0; i < 8; i++) { NVIC->IP[i] = NVIC_IPR_saved[i]; }
...
uint8_t BlueNRG_Sleep(SleepModes sleepMode, uint8_t gpioWakeBitMask, uint8_t gpioWakeLevelMask)
{
  ...
  {
    /* Store the watchdog configuration and the disable it to avoid reset during
     * CPU halt. */
    uint32_t WDG_CR_saved = WDG->CR;
    WDG->CR               = 0;

    /* Wait for interrupt is called: the core execution is halted until an event
     * occurs. */
    __WFI();
