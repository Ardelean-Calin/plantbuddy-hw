...
static uint8_t CondRoutineTrue(ActionPacket* p) { return TRUE; }
...
/**
 * @brief  This routine sets the network ID field for packet transmission and
 * filtering for the receiving. Only two devices with same networkID can
 ...
 *           4. It shall have a minimum of two transitions in the most
...
/**
 * @brief  This routine sends a packet on a specific channel and at a specific
 * time.
 * @param  channel: Frequency channel between 0 to 39.
 * @param  wakeup_time: Time of transmission in us. This is relative time
 * regarding now. The relative time cannot be less than 100 us otherwise it
 * might wrap around after 32.7 second.
 * @param  txBuffer: Pointer to TX data buffer. Second byte of this buffer must
 * be the length of the data.
 * @param  Callback: This function is being called as data routine.
 *         First ActionPacket is current action packet and the second one is
 * next action packet.
 * @retval uint8_t return value
 *           - 0x00 : Success.
 *           - 0xC0 : Invalid parameter.
...
/**
 * @brief  This routine sends a packet on a specific channel and at a certain
 * time then wait for receiving acknowledge.
 * @param  channel: Frequency channel between 0 to 39.
 * @param  wakeup_time: Time of transmission based on us. This is relative time
 * regarding now. The relative time cannot be less than 100 us otherwise it
 * might wrap around after 32.7 second.
 * @param  txBuffer: Pointer to TX data buffer. Secound byte of this buffer must
 * be the length of the data.
 * @param  rxBuffer: Pointer to RX data buffer. Secound byte of this buffer must
 * be the length of the data.
...
uint8_t HAL_RADIO_SendPacketWithAck(uint8_t channel, uint32_t wakeup_time, uint8_t* txBuffer,
                                    uint8_t* rxBuffer, uint32_t receive_timeout,
                                    uint8_t (*Callback)(ActionPacket*, ActionPacket*))
{
  uint8_t  returnValue = SUCCESS_0;
  uint32_t dummy, time;
...
/**
 * @brief  This routine receives a packet on a specific channel and at a certain
 * time.
 * @param  channel: Frequency channel between 0 to 39.
 * @param  wakeup_time: Time of transmission based on us. This is relative time
 * regarding now. The relative time cannot be less than 100 us otherwise it
 ...
 *           - 0xC4 : Radio is busy, receiving has not been triggered.
 */
uint8_t HAL_RADIO_ReceivePacket(uint8_t channel, uint32_t wakeup_time, uint8_t* rxBuffer,
...
/**
 * @brief  This routine receives a packet on a specific channel and at a certain
 * time. Then sends a packet as an acknowledgment.
 * @param  channel: frequency channel between 0 to 39.
 * @param  wakeup_time: time of transmission based on us. This is relative time
 * regarding now. The relative time cannot be less than 100 us otherwise it
 * might wrap around after 32.7 second.
 * @param  rxBuffer: points to received data buffer. second byte of this buffer
 * determines the length of the data.
 * @param  txBuffer: points to data buffer to send. secound byte of this buffer
 * must be the length of the buffer.
 * @param  receive_timeout: Time of RX window used to wait for the packet on us.
 * @param  callback: This function is being called as data routine.
 *         First ActionPacket is current action packet and the second one is
 * next action packet.
...
uint8_t HAL_RADIO_ReceivePacketWithAck(uint8_t channel, uint32_t wakeup_time, uint8_t* rxBuffer,
                                       uint8_t* txBuffer, uint32_t receive_timeout,
                                       uint8_t (*Callback)(ActionPacket*, ActionPacket*))
{
  uint8_t  returnValue = SUCCESS_0;
  uint32_t dummy, time;
