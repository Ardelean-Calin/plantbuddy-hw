...
#define ADVANCE_QUEUE(index, size, max_size) \
  ((index + size) % max_size) /* (((index + size) >= max_size) ? 0 : (index + size))*/
#define FIFO_ALIGNMENT fifo->alignment
#define FIFO_GET_SIZE(fifo)                               \
  ((fifo->tail >= fifo->head) ? (fifo->tail - fifo->head) \
                              : (fifo->max_size - (fifo->head - fifo->tail)))
 ...
 * so that no wrapping occurs and each element is made of only linear buffer
 * segments.
 * @retval None
 */
...
uint16_t fifo_size(circular_fifo_t* fifo) { return FIFO_GET_SIZE(fifo); }
void     fifo_flush(circular_fifo_t* fifo) { fifo->tail = fifo->head = 0; }
...
// static uint8_t fifo_put_no_wrap(circular_fifo_t *fifo, uint16_t size, uint8_t
// *buffer)
//{
