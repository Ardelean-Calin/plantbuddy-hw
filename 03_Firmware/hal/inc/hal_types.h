...
/** @brief Macro that stores a 16-bit value into a buffer in Little Endian
 * Format (2 bytes)*/
#define HOST_TO_LE_16(buf, val) (((buf)[0] = (uint8_t)(val)), ((buf)[1] = (uint8_t)(val >> 8)))

/** @brief Macro that returns a 16-bit value from a buffer where the value is
 * stored in Little Endian Format */
#define LE_TO_HOST_16(ptr) \
  (uint16_t)(((uint16_t) * ((uint8_t*)ptr)) | ((uint16_t) * ((uint8_t*)ptr + 1) << 8))

/** @brief Macro that returns a value from a 32-bit buffer where the value is
 * stored in Little Endian Format */
#define LE_TO_HOST_32(ptr)                                                              \
  (uint32_t)(((uint32_t) * ((uint8_t*)ptr)) | ((uint32_t) * ((uint8_t*)ptr + 1) << 8) | \
             ((uint32_t) * ((uint8_t*)ptr + 2) << 16) | ((uint32_t) * ((uint8_t*)ptr + 3) << 24))

/** @brief Macro that stores a 32-bit value into a buffer in Little Endian
 * Format (4 bytes) */
