...
#include <stdint.h>
  ...
  typedef struct VTIMER_HandleTypeS
  {
    ...
    BOOL                active;      /*Managed by the timer tick. User does not have to care about
                                        this*/
    struct VTIMER_HandleTypeS* next; /*Managed by the timer tick. User does not
  ...
  {
    uint64_t expiryTime; /*When the first bit is transmitted or when the receive
                            window is opened*/
    BOOL cal_req;
    BOOL active;  /*Managed by the timer tick. User does not have to care about
                     this*/
    BOOL pending; /*Managed by the timer tick. User does not have to care about
                     this*/
    BOOL event_type;
  ...
  typedef struct HAL_VTIMER_InitS
  {
    uint16_t XTAL_StartupTime; /*!< XTAL startup in 2.44 us unit */
                                ...
                                * periodic                            calibration is active (PeriodicCalibrationInterval != 0).
                                */
    BOOL     EnableInitialCalibration;
    uint16_t PeriodicCalibrationInterval; /*!< Periodic calibration interval in
...
#define HAL_VTIMER_RX_EVENT          (0x00U)
  ...
  /**
   * @brief Returns the difference between two absolute times:
   * sysTime1-sysTime2. The resulting value is expressed in ms.
  ...
  /**
   * @brief  Returns the admitted sleep mode according to the next timer
   * activity.
   * @return Sleep mode
   */
  ...
  /**
   * @brief Virtual timer Timeout Callback. It must be called in the Blue
   * handler if the host wake up timer bit in the radio controller
   * interrupt/status register is active.
   * @retval None
   */
   ...
   * expressed in STU If the calibration of the low speed oscillator is needed,
   * if it is possible, the radio timer will be programmed with the latest
   * calibration data.
   * @param time: Absolute time expressed in STU.
   ...
   *        The timeout of the last radio timer activity taken into account by
   * the Timer Module is saved in the variable passed as parameter.
  ...
  /**
   * @brief Clear the last radio activity scheduled disabling the radio timer
   * too. Furthermore, it returns if the timeout is too close with respect the
