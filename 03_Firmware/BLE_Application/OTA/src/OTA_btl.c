 ...
 * @brief   BLE Over The Air (OTA) FW upgrade implementation
 ******************************************************************************
 * @attention
 *
...
/* BLE OTA Server with data length estension version */
#define BLE_OTA_DLE_SERVER_VERSION 2
...
#define DLE_ATT_MTU_DONE \
  (DLE_DONE | ATT_MTU_DONE) // OTA Client has performed bothe DLE and ATT_MTU exchange
...
#define NOTIFICATION_INTERVAL(x) \
  (((x) == 1) || ((x) == 3))     \
      ? 1                        \
      : (NOTIFICATION_WINDOW) // 3: backward compatibility with old OTA client TBR

/* The following defines **MUST NOT** be modified for proper operation of the current OTA BTL
 * release */
#define PAGE_SIZE (2048) // Flash page size
#define BYTE_INCREMENT \
  (16) // It's equal to max flash size we can write: 16 bytes with Flash Burst Write
 ...
 */
#define MAX_NUM_BLOCKS_X_PACKET ((OTA_ATT_MTU_SIZE - 3 - 4) / (BYTE_INCREMENT))

/* OTA FW image content MAX size related to the max supported OTA_ATT_MTU_SIZE from OTA Server */
#define IMAGE_CONTENT_SIZE                      \
  ((BYTE_INCREMENT * MAX_NUM_BLOCKS_X_PACKET) + \
   4) /* + 4 for (OTA sequence number, check sum needs ack) */

/* MAX buffer size to hold only the image file data on received notification/s and to be written on
   flash: It is related to the max supported OTA_ATT_MTU_SIZE from OTA Server */
#define BUF_SIZE                                      \
  (((BYTE_INCREMENT * MAX_NUM_BLOCKS_X_PACKET) + 4) * \
   NOTIFICATION_WINDOW) // STATIC value tailored for MAX default value for OTA_ATT_MTU_SIZE

/**** OTA macros for defining actual OTA FW transfer paramaters related to actual OTA att mtu size
 * agreed with OTA Client *****************/

/* Set actual NUM_BLOCKS_X_PACKET used on OTA FW data transfer:
   OTA packet actual size = 4 bytes (OTA sequence number, check sum needs ack) +
   (num_block_x_packets * BYTE_INCREMENT) bytes in a single data packet */
#define NUM_BLOCKS_X_PACKET(ota_att_mtu_size) (((ota_att_mtu_size)-4) / BYTE_INCREMENT)

/* Flash burst write guard time for BYTE_INCREMENT (16) * num_block_x_packets(ota_att_mtu_size)  *
 * NOTIFICATION_WINDOW (8 writes) bytes */
#define OTA_WRITE_GUARD_TIME(ota_att_mtu_size)                                         \
  (((((((((4 + (NUM_BLOCKS_X_PACKET(ota_att_mtu_size) * 16)) * NOTIFICATION_WINDOW)) / \
        NUM_BURST_WRITE_BYTES)) *                                                      \
      BURST_WRITE_BYTES_uS_TIME) /                                                     \
     (1000))) +                                                                        \
   1)

#if (OTA_EXTENDED_PACKET_LEN == 1)
/* Min required Connection interval, with extended packet length enabled, for handling required
 * Flash write operations when no radio activity */
#define OTA_EXT_LE_L2CAP_CONN_INTERVAL(ota_att_mtu_size)   \
  (((OTA_WRITE_GUARD_TIME(ota_att_mtu_size) + 4) * 1000) / \
   1250) // Added further margin for BLE communication
#endif
...
#define BUFPOINTER_LIMIT(ota_att_mtu_size) \
  (DATA_PACKET_SIZE(ota_att_mtu_size) * NOTIFICATION_WINDOW)
...
uint16_t btlServHandle, btlImageCharHandle, btlNewImageCharHandle, btlNewImageTUContentCharHandle,
...
#define COPY_BTL_SERVICE_UUID(uuid_struct) \
  COPY_UUID_128(uuid_struct,               \
                ...
                0x9a,                      \
                0x66)
#define COPY_IMAGE_CHAR_UUID(uuid_struct) \
  COPY_UUID_128(uuid_struct,              \
                0x12,                     \
                0x2e,                     \
                0x8c,                     \
                ...
                0x9a,                     \
                0x66)
#define COPY_NEW_IMAGE_CHAR_UUID(uuid_struct) \
  COPY_UUID_128(uuid_struct,                  \
                0x21,                         \
                0x0f,                         \
                0x99,                         \
                ...
                0x9a,                         \
                0x66)
#define COPY_NEW_IMAGE_TU_CONTENT_CHAR_UUID(uuid_struct) \
  COPY_UUID_128(uuid_struct,                             \
                0x26,                                    \
                0x91,                                    \
                0xaa,                                    \
                ...
                0x9a,                                    \
                0x66)
#define COPY_EXPECTED_IMAGE_TU_SEQNUM_CHAR_UUID(uuid_struct) \
  COPY_UUID_128(uuid_struct,                                 \
                0x2b,                                        \
                0xdc,                                        \
                0x57,                                        \
...
static uint8_t imageBuffer[BUF_SIZE];

uint8_t BTLServiceUUID4Scan[18] = {0x11,
                                   0x06,
                                   0x8a,
                                   0x97,
                                   0xf7,
...
/* Let the application know whether we are in the middle of a bootloading session through a global
 * status variable */
 ...
 * @note The API code could be subject to change in future releases.
 */
void OTA_terminate_connection(void) { bootloadingCompleted_end = ota_allow_jump; }
...
static void OTA_Set_Validity_Tags(void)
{
  ...
  if (OTA_OP_CODE ==
      OTA_APP_SWITCH_OP_CODE_GO_TO_LOWER_APP) // Lower Application OTA done with success
  {
    /* Set valid tag x lower application (the new application just successfully upgraded through
     * OTA) */
    ...
    OTA_Set_Application_Tag_Value(APP_HIGHER_ADDRESS, OTA_INVALID_OLD_TAG);
           ...
           OTA_APP_SWITCH_OP_CODE_GO_TO_HIGHER_APP) // Higher Application OTA done with success
  {
    /* Set valid tag x higher application (the new application just successfully upgraded through
     * OTA) */
  ...
  else if (OTA_OP_CODE == OTA_APP_SWITCH_OP_CODE_GO_TO_NEW_APP) // OTA Service Manager has upgraded
                                                                // a new application with success
  {
...
tBleStatus OTA_Add_Btl_Service(void)
{
  ...
  uint8_t    uuid[16];
  uint8_t    imageinfo[9] = {
      BLE_OTA_DLE_SERVER_VERSION, 0, 0, 0, 0, 0, 0, 0, 0}; // BLE OTA DLE server version
  ...
  ret = aci_gatt_add_char(btlServHandle,
                          UUID_TYPE_128,
                          &ota_char_uuid,
                          8,
                          CHAR_PROP_READ,
  ...
  Osal_MemCpy(&ota_char_uuid.Char_UUID_128, uuid, 16);
  /* 2ND OTA SERVICE CHARAC ALLOCATION: BASE AND SIZE OF NEW IMAGE + NOTIFICATION WINDOW/DLE version
   */
  ret = aci_gatt_add_char(btlServHandle,
                          UUID_TYPE_128,
                          &ota_char_uuid,
  ...
  /*  We provides info about the BLE OTA DLE server version */
  ret = aci_gatt_update_char_value_ext(
      conn_handle, btlServHandle, btlNewImageCharHandle, 1, 9, 0, 9, (uint8_t*)imageinfo);
  if (ret != BLE_STATUS_SUCCESS)
  {
  ...
  Osal_MemCpy(&ota_char_uuid.Char_UUID_128, uuid, 16);
  /* 4TH OTA SERVICE CHARAC ALLOCATION: IMAGE BLOCK FOR NOTIFICATION, INCLUDES NEXT EXPECTED IMAGE
   * NUMBER AND ERROR CONDITIONS */
  ret = aci_gatt_add_char(btlServHandle,
                          UUID_TYPE_128,
                          &ota_char_uuid,
                          4,
  ...
  PRINTF("\r\n");
#endif
  /* Free flash space advertised by the application (data for 1ST OTA BTL characteristic
   * initialization) */
  ...
  ret = aci_gatt_update_char_value_ext(
      conn_handle, btlServHandle, btlImageCharHandle, 1, 8, 0, 8, (uint8_t*)currentImageInfos);
  if (ret != BLE_STATUS_SUCCESS)
  {
    PRINTF("Error while updating characteristic.\n");
    return BLE_STATUS_ERROR;
...
void OTA_Write_Data(void)
{
  ...
  /* store the potential last sequence number successfully written (if no flash write errors are
   * detected) */
  ...
  while (k < bufPointer)
  {
    ...
    if (currentWriteAddress != (imageBase + OTA_TAG_VECTOR_TABLE_ENTRY_OFFSET))
    {
      ...
      if ((bufPointer - k) >= BYTE_INCREMENT)
      {
        ...
        verifyStatus =
            FLASH_Verify(currentWriteAddress, (uint32_t*)(&imageBuffer[k]), BYTE_INCREMENT);
      }
      else
      {
        uint8_t byteIncrement = 4; /* Flash Write: 1 word (4 bytes) */
        FLASH_ProgramWord(currentWriteAddress,
                          (((uint32_t)imageBuffer[k + 3] << 24) +
                           ((uint32_t)imageBuffer[k + 2] << 16) +
                           ((uint32_t)imageBuffer[k + 1] << 8) + (uint32_t)imageBuffer[k]));
        verifyStatus =
            FLASH_Verify(currentWriteAddress, (uint32_t*)(&imageBuffer[k]), byteIncrement);
      }
  ...
  /* prepare notification data for next expected block if for both success or notify write/verify
   * failure */
  if (verifyStatus == SUCCESS)
  {
 ...
 * @note The API code could be subject to change in future releases.
 */
void OTA_Write_Request_CB(uint16_t connection_handle, uint16_t attr_handle, uint8_t data_length,
                          uint8_t* att_data)
{
  tBleStatus ret;
  uint16_t   k;
  ...
  if (attr_handle == (btlNewImageCharHandle + 1))
  {
     ...
     */
    imageSize = (uint32_t)(att_data[4] << 24) + (uint32_t)(att_data[3] << 16) +
                (uint32_t)(att_data[2] << 8) + att_data[1];
    imageBase = (uint32_t)(att_data[8] << 24) + (uint32_t)(att_data[7] << 16) +
                (uint32_t)(att_data[6] << 8) + att_data[5];
  ...
  else if (attr_handle == (btlExpectedImageTUSeqNumberCharHandle + 2))
  {
    ...
    if (client_DLE_ATT_MTU != DLE_ATT_MTU_DONE)
    {
      ...
      ota_att_mtu_size = DEFAULT_ATT_MTU;
    }
  }
  else if (attr_handle == (btlNewImageTUContentCharHandle + 1))
  {
    ...
    if (detected_error)
    {
      ...
      write_counter += 1;
      /* When the next expected notification from OTA client must be sent sent (inline with
         notification window), the detected error code is notified with the sequence number to be
         used for retrying again all the writes of this block */
      OTA_Check_Update_Error_Condition(notification_range);
    }
    /* Here we read updated characteristic content filled by "write with no response command' coming
     * from the master */
    else if (bufPointer < BUFPOINTER_LIMIT(ota_att_mtu_size))
    {
      // else if (bufPointer < bufPointer_limit){
      /* Data will be received by the OTA slave 16 byte wise (due to characteristic image content =
       * 16 bytes image + 4 of headers) Drop new image data into buffer
       */

      for (k = bufPointer; k < (bufPointer + data_length - 4); k++)
      { // Store 16 bytes of received notification on imageBuffer
        if (k < imageSize)
      ...
      checksum ^=
          (att_data[data_length - 3] ^ att_data[data_length - 2] ^ att_data[data_length - 1]);
      bufPointer = k;
      /* In the section of code below: notify for received packet integrity (cheksum), sequence
       * number correctness and eventually write flash (which will get notified as well)
       */
      ...
      if (checksum == att_data[0])
      {
        ...
        receivedSeqNum = ((att_data[DATA_PACKET_SIZE(ota_att_mtu_size) + 3] << 8) +
                          att_data[DATA_PACKET_SIZE(ota_att_mtu_size) + 2]);

        if (expectedSeqNum == receivedSeqNum)
        {
          /* sequence number check ok, increment expected sequence number and prepare for next block
           * notification */
          expectedSeqNum++;

          if ((att_data[DATA_PACKET_SIZE(ota_att_mtu_size) + 1] == 1) ||
              (((receivedSeqNum + 1) * DATA_PACKET_SIZE(ota_att_mtu_size)) >=
               imageSize)) // TBR ((receivedSeqNum+1) % notification_range) or
                           // (att_data[NEEDS_ACK_INDEX] == 1)
          {
            /* Here is where we manage notifications related to correct sequence number and
             * write/verify results if conditions get us through the next nested 'if' section (FLASH
             * write section)
             */
            ...
            if (((bufPointer % BUFPOINTER_LIMIT(ota_att_mtu_size)) == 0) ||
                (((imageSize - totalBytesWritten) < BUFPOINTER_LIMIT(ota_att_mtu_size)) &&
                 (bufPointer >= (imageSize - totalBytesWritten))))
            {
              ota_write_data = 1;
        ...
        else
        {
          ...
          expectedSeqNum = (last_written_receivedSequence == 0)
                               ? last_written_receivedSequence
                               : (last_written_receivedSequence + 1);

          /* Set error flags for sequence number error */
          OTA_Set_Error_Flags(OTA_SEQUENCE_ERROR, expectedSeqNum);

          /* null packet due to seq failure: remove from internal buffer through buffer pointer
           * shift */
          bufPointer = 0;
#ifdef ST_OTA_BTL_MINIMAL_ECHO
          PRINTF(
              "Sequence number check failed, expected frame # 0x%02X but 0x%02X was received \r\n",
              expectedSeqNum,
              receivedSeqNum);
#endif
          /* An error has been detected just on last write of current notification window: the
             detected error code can be notified now since OTA client is ready to get the expected
             notification. Notification is done with the sequence number to be used for retrying
             again all the writes of this block */
          OTA_Check_Update_Error_Condition(notification_range);
        }
      } /* if check sum */
      else
      {
        ...
        /* set new expected sequence number */
        expectedSeqNum = (last_written_receivedSequence == 0) ? last_written_receivedSequence
                                                              : (last_written_receivedSequence + 1);
        ...
        /* An error has been detected just on last write of current notification window: the
           detected error code can be notified now since OTA client is ready to get the expected
           notification.
           Notification is done with the sequence number to be used for retrying again all the
           writes of this block */
        OTA_Check_Update_Error_Condition(notification_range);
      }
      checksum = 0;
    }
...
{
  /* Check if data for flash write are ready: do write only if there is enough time before next
   * radio activity */
  if (ota_write_data)
  {
    /* Data buffer are available for Flash write */
    if (HAL_VTimerDiff_ms_sysT32(Next_State_SysTime, HAL_VTimerGetCurrentTime_sysT32()) >
        OTA_WRITE_GUARD_TIME(ota_att_mtu_size))
    {
      ota_write_data = 0;
      OTA_Write_Data();
    }
...
{
  /* If OTA Client supports extended data length, this event is raised: set associated flag and
     increase connection interval for allowing OTA transfer with data length extension */

  client_DLE_ATT_MTU |= DLE_DONE;
  /* Increase connection interval for handling BUF_SIZE Flash write operations when no radio
   * activity */
  aci_l2cap_connection_parameter_update_req(
      Connection_Handle,
      (uint16_t)OTA_EXT_LE_L2CAP_CONN_INTERVAL(ota_att_mtu_size),
      (uint16_t)OTA_EXT_LE_L2CAP_CONN_INTERVAL(ota_att_mtu_size),
      0,
      0xC80);
}
